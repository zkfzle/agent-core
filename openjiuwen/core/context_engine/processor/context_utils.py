...
from openjiuwen.core.foundation.llm1 import BaseMessage, InferenceAffinityModel
...
class ContextUtils:
    ...
    @staticmethod
    def release_context_in_model(model: Any,
                                 session_id: str,
                                 messages: List[BaseMessage],
                                 message_release_index: int,
                                 *,
                                 tools: List = None,
                                 tool_release_index: int = 0
                                 ):
        if not model or not isinstance(model, InferenceAffinityModel):
            return
        ...
        result = model.release(
            ...
            messages=messages,
            messages_released_index=message_release_index,
            tools=tools,
            tools_released_index=tool_release_index
        )
