    ...
    def parse():
        try:
            if suffix == ".json":
                ...
                raise JiuWenBaseException(
                    error_code=StatusCode.PLUGIN_UNEXPECTED_ERROR.code, message=f"Only supports. json/. yaml/. yml, current extension: {suffix}"
                )
        except Exception as e:
            raise JiuWenBaseException(error_code=StatusCode.PLUGIN_UNEXPECTED_ERROR.code, message=f"{e}")
