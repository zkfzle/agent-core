...
class TestMemoryEngine(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_memory_engine(self):
        ...
        self.assertTrue(recent_message is None)
        ...
        test_msg5 = BaseMessage(role="user",
                                content="我来自杭州")
        input_messages = [[test_msg1], [test_msg2], [test_msg3], [test_msg4], [test_msg5]]
        message_id = "-1"
        # add
        ...
        self.assertTrue(recent_message[0].content == test_msg5.content)
        ...
        print(f"get_test_variable: {test_variable}")
        ...
        if user_profile:
            for mem in user_profile:
                print(f"after update user profile: {mem['mem']}")
        ...
        self.assertTrue(test_variable['年龄'].find("20") != -1)
        ...
        self.assertTrue("年龄" not in test_variable)
