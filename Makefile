...
ifeq ($(UV_EXISTS),yes)
	...
	@$(EXEC_CMD)uv pip install $(DEPENDENCIES)
else
	@echo [Makefile] Installing dependencies via pip
	@$(EXEC_CMD)pip install $(DEPENDENCIES)
endif
...
format: has-staged-changes
	-@$(EXEC_CMD)ruff format --check $(CHANGED_FILES)

lint: has-staged-changes
	-@$(EXEC_CMD)ruff check --show-fixes $(CHANGED_FILES)

pylint: has-staged-changes
	-@$(EXEC_CMD)pylint $(CHANGED_FILES)

spelling: has-staged-changes
	-@$(EXEC_CMD)codespell $(CHANGED_FILES)

fix-format: has-staged-changes
	-@$(EXEC_CMD)ruff format $(CHANGED_FILES)

fix-lint: has-staged-changes
	-@$(EXEC_CMD)ruff check --fix $(CHANGED_FILES)
