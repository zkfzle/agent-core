...
class ToolMgr:
    ...
    async def add_tool_server(self, server_config: McpServerConfig, expiry_time: Optional[float] = None) -> List[
        ...
        try:
            ...
            if not connected:
                raise build_error(StatusCode.RESOURCE_MCP_SERVER_CONNECTION_ERROR, server_config=server_config, reason="")
    ...
    async def remove_tool_server(self, server_id: str, ignore_not_exist: bool = True) -> list[str]:
        ...
        if not mcp_server_resource:
            if not ignore_not_exist:
                raise build_error(StatusCode.RESOURCE_MCP_SERVER_REMOVE_ERROR, server_id=server_id, reason="server is not exist")
            else:
                return []
        try:
    ...
    async def refresh_tool_server(self, server_id: str, skip_not_exist: bool = False, force: bool = False):
        ...
        if need_refresh:
            await self._inner_refresh_mcp_tools(mcp_resource.client, mcp_resource.config, mcp_resource.expiry_time)
