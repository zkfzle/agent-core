...
class TestLongTermMemory(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        embed_model = APIEmbedding(embed_config)

        # ---------- KV Store ----------
        kv_store = DbmKVStore("./dbmstore.db")
        ...
        # )
        vector_store = ChromaVectorStore(persist_directory="./resource_dir")

        # ---------- semantic_store ----------
        semantic_store = SemanticStore(vector_store=vector_store, embedding_model=embed_model)
    ...
    async def test_delete_scope(self):
        # First set up a scope to delete
        scope_id = "app0108_1"
        user_id = "user0108_1"
        ...
        user_profile_before = await self.engine.get_user_mem_by_page(user_id=user_id, scope_id=scope_id,
                                                                     page_size=10, page_idx=1, type=MemoryType.UNKNOWN)
        ...
        # Verify scope is removed from memory cache
        self.assertNotIn(scope_id, self.engine._scope_config)
        self.assertNotIn(scope_id, self.engine._scope_embedding)
    ...
    async def test_add_messages(self):
        ...
        # Search for the occupation profile using search_user_mem since it's not in the initial get_user_mem_by_page results
