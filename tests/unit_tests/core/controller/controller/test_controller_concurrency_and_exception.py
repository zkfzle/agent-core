...
def build_normal_executor(config, ability_manager, context_engine, task_manager, event_queue):
    """Build normal task executor"""
    return NormalTaskExecutor(config, ability_manager, context_engine, task_manager, event_queue)
...
def build_stream_exception_executor(config, ability_manager, context_engine, task_manager, event_queue):
    """Build stream-exception task executor"""
    return ExceptionInStreamTaskExecutor(config, ability_manager, context_engine, task_manager, event_queue)


def build_slow_executor(config, ability_manager, context_engine, task_manager, event_queue, sleep_time: float = 10.0):
    """Builder function for SlowTaskExecutor"""
    return SlowTaskExecutor(config, ability_manager, context_engine, task_manager, event_queue, sleep_time)
...
class TestConcurrentSessionIsolation:
    ...
    async def test_session_task_isolation(self):
        ...
        output_2 = results[1]
        assert not isinstance(output_2, Exception), "Session 2 should not raise exception (task failure != session failure)"

        # Verify failed tasks of Session 2
        failed_tasks = [t for t in agent_2.controller._task_manager.get_task(task_filter=None)
                       if t.status == TaskStatus.FAILED]
        assert len(failed_tasks) > 0, "Session 2 should have failed tasks"

        # Verify Session 1 has no failed tasks
        all_tasks_1 = agent_1.controller._task_manager.get_task(task_filter=None)
...
class TestExceptionHandling:
    ...
    async def test_stream_output_exception_isolation(self):
        ...
        assert normal_task is not None, "Normal task should exist"
        assert normal_task.status == TaskStatus.COMPLETED, \
            f"Normal task should be completed (not affected by stream output exception), actual status is {normal_task.status}"
...
class SlowTaskExecutor(TaskExecutor):
    ...
    def __init__(self, config, ability_manager, context_engine, task_manager, event_queue, sleep_time: float = 10.0):
        super().__init__(config, ability_manager, context_engine, task_manager, event_queue)
    ...
    async def execute_ability(self, task_id: str, session: Session) -> AsyncIterator[ControllerOutputChunk]:
        ...
        yield ControllerOutputChunk(
            ...
            payload=ControllerOutputPayload(
                type="processing",
                data=[TextDataFrame(type="text", text=f"Slow task {task_id} started, will sleep for {self.sleep_time}s")]
            ),
            last_chunk=False
        )
...
class TestControllerConfig:
    ...
    async def test_task_timeout(self):
        ...
        agent = await build_test_agent(
            ...
            task_executors={"slow": lambda cfg, am, ce, tm, eq: build_slow_executor(cfg, am, ce, tm, eq, sleep_time=10.0)}
        )
        
        # Configure timeout
        agent.controller._task_scheduler._config.task_timeout = 2.0  # 2 seconds
        agent.controller._task_scheduler._config.schedule_interval = 0.1
        
        session = TaskSession(trace_id="test_timeout_session")
        input_event = InputEvent(
            event_type=EventType.INPUT,
        ...
        try:
            ...
            # Check task status
            task_manager = agent.controller._task_scheduler._task_manager
            tasks = task_manager.get_task(task_filter=TaskFilter(session_id=session.session_id()))
    ...
    async def test_timeout_vs_manual_cancel(self):
        ...
        agent.controller._task_scheduler._config.task_timeout = 10.0
        agent.controller._task_scheduler._config.schedule_interval = 0.1
        
        session = TaskSession(trace_id="test_cancel_session")
        input_event = InputEvent(
            event_type=EventType.INPUT,
        ...
        try:
            ...
            success = await agent.controller._task_scheduler.cancel_task(task_id, session)
            assert success, "Manual cancellation should succeed"
            
            await asyncio.sleep(0.5)
            
            # Verify task is CANCELED (not FAILED from timeout)
            tasks = task_manager.get_task(task_filter=TaskFilter(task_id=task_id))
    ...
    async def test_max_concurrent_tasks_limit(self):
        ...
        Test objectives:
        1. Start multiple tasks concurrently
        2. Verify tasks beyond configured max concurrency wait until previous ones complete
        3. Producer side can actively cancel the task successfully and session is not blocked (stream_task.cancel())
        """
        agent = await build_test_agent(
            agent_id="test_max_concurrent",
            event_handler=ConcurrentTasksEventHandler(),
            task_executors={"normal": build_normal_executor}
        )

        scheduler = agent.controller._task_scheduler
        scheduler._config.max_concurrent_tasks = 1
        scheduler._config.schedule_interval = 0.1

        session = TaskSession(trace_id="test_max_concurrent_tasks")
        input_event = InputEvent(
            event_type=EventType.INPUT,
            content={"query": "test max_concurrent_tasks"},
        )
        ...
        try:
            for _ in range(10):
                await asyncio.sleep(0.1)
                running_count = len(agent.controller._task_scheduler._running_tasks)
                if running_count > 0:
                    break

            running_count = len(agent.controller._task_scheduler._running_tasks)
            logger.info(f"Number of concurrently running tasks: {running_count}")
            assert (
                running_count <= 1
            ), f"The number of concurrently running tasks should not exceed 1, actually {running_count}"
        finally:
            stream_task.cancel()
            try:
                await stream_task
                ...
                pass
            await agent.controller.stop()
        logger.info("✅ test_max_concurrent_tasks_limit passed")
