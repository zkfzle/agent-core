...
class TaskFilter(BaseModel):
    ...
    def validate_at_least_one_filter(self):
        ...
        if (self.task_id is None and 
            self.session_id is None and 
            self.user_id is None and 
            self.priority is None and 
            self.status is None and 
            not self.is_root):
            raise JiuWenBaseException(
                StatusCode.CONTROLLER_RUNTIME_ERROR.code,
                StatusCode.CONTROLLER_RUNTIME_ERROR.errmsg.format(
                    error_msg="At least one filter parameter (task_id, session_id, user_id, priority, status, or is_root) must be provided"
                )
            )
        return self
...
class TaskManager:
    ...
    def get_task(
        ...
        if task_filter.priority is not None:
            ...
            if task_filter.priority == "highest":
                raise JiuWenBaseException(
                    StatusCode.CONTROLLER_TASK_EXISTS_ERROR.code,
                    StatusCode.CONTROLLER_TASK_EXISTS_ERROR.errmsg.format(
    ...
    def remove_task(
        ...
        if task_filter.task_id is None and task_filter.session_id is None and task_filter.user_id is None and task_filter.priority is None and task_filter.status is None and not task_filter.is_root:
            raise JiuWenBaseException(
                StatusCode.CONTROLLER_REMOVE_TASK_ERROR.code,
                StatusCode.CONTROLLER_REMOVE_TASK_ERROR.errmsg.format(
                    error_msg="Cannot remove tasks: task_id is None and no other filter criteria provided. "

                )
            )

        # Get tasks to remove
        # Convert priority: "highest" is not supported in remove_task, string representations should be converted to int
        ...
        tasks_to_remove = self.get_task(task_filter=filter_for_get)
