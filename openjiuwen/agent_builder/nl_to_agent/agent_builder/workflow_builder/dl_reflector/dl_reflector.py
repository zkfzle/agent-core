...
class Reflector:
    def __init__(self):
        ...
        self.variable_types = ['String', 'Integer', 'Number', 'Boolean', 'Object',
    ...
    def _check_questioner_node(self, node_content: dict):
        ...
        if configs_format_has_error:
            return
        configs_keys = node_content['parameters']['configs'].keys()
        if 'prompt' not in configs_keys:
            self.errors.append(f"{node_content['id']}节点, 类型为{node_content['type']}, 'parameters'中的'configs'字典中缺失'prompt'属性")
    ...
    def _check_plugin_node(self, node_content: dict):
        ...
        if configs_format_has_error:
            return
        configs_keys = node_content['parameters']['configs'].keys()
        if 'tool_id' not in configs_keys:
            self.errors.append(f"{node_content['id']}节点, 类型为{node_content['type']}, 'parameters'中的'configs'字典中缺失'tool_id'属性")
    ...
    def _check_code_node(self, node_content: dict):
        ...
        if configs_format_has_error:
            return
        configs_keys = node_content['parameters']['configs'].keys()
        if 'code' not in configs_keys:
            self.errors.append(f"{node_content['id']}节点, 类型为{node_content['type']}, 'parameters'中的'configs'字典中缺失'code'属性")
        self._check_next_missing(node_content)
    ...
    def _check_inputs_list(self, node_content: dict, check_type: bool = False):
        ...
        if 'inputs' not in node_content['parameters']:
            ...
            return
        if isinstance(node_content['parameters']['inputs'], list):
        ...
        for input_item in node_content['parameters']['inputs']:
            ...
            if check_type:
                if 'type' not in input_item:
                    ...
                    if input_item['type'] not in self.variable_types:
                        self.errors.append(f"{node_content['id']}节点, 类型为{node_content['type']}, 'parameters'中的'inputs'列表中的元素'type'属性值必须为{self.variable_types}中的一个")
    ...
    def _check_outputs_list(self, node_content: dict, check_type: bool = False):
        ...
        if 'outputs' not in node_content['parameters']:
            ...
            return
        if isinstance(node_content['parameters']['outputs'], list):
        ...
        for output_item in node_content['parameters']['outputs']:
            ...
            if check_type:
                if 'type' not in output_item:
                    ...
                    if output_item['type'] not in self.variable_types:
                        self.errors.append(f"{node_content['id']}节点, 类型为{node_content['type']}, 'parameters'中的'outputs'列表中的元素'type'属性值必须为{self.variable_types}中的一个")

    def _check_configs_format(self, node_content: dict) -> bool:
