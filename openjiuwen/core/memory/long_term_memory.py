...
class LongTermMemory(metaclass=Singleton):
    ...
    async def register_store(self, kv_store: BaseKVStore,
        ...
        if kv_store is None:
            raise build_error(
                StatusCode.MEMORY_STORE_REGISTER_FAILED,
                store_type="kv store",
    ...
    def set_config(self, config: MemoryEngineConfig):
        ...
        if not self.kv_store or not self.semantic_store or not self.db_store:
            raise build_error(
                StatusCode.MEMORY_SET_CONFIG_OPERATION_FAILED,
                config_type="system",
    ...
    async def add_messages(
        ...
        async with lock:
            ...
            try:
                ...
                raise build_error(
                    StatusCode.MEMORY_ADD_MEMORY_OPERATION_FAILED,
                    memory_type="unknown",
                    error_msg=f"{str(e)}",
                    cause=e
                )
    ...
    async def delete_mem_by_id(self,
        ...
        async with lock:
            if not self.write_manager:
                raise build_error(
                    StatusCode.MEMORY_DELETE_MEMORY_OPERATION_FAILED,
                    memory_type="all",
    ...
    async def update_mem_by_id(self,
        ...
        async with lock:
            if not self.write_manager:
                raise build_error(
                    StatusCode.MEMORY_UPDATE_MEMORY_OPERATION_FAILED,
                    memory_type="all",
                    error_msg=f"write manager is not initialized",
                )
    ...
    async def get_variables(self,
        ...
        if not self.search_manager:
            raise build_error(
                StatusCode.MEMORY_GET_MEMORY_OPERATION_FAILED,
                memory_type="all",
    ...
    async def get_user_mem_by_page(self,
        ...
        for item in search_data:
            mem_type = item.get("mem_type", MemoryType.USER_PROFILE.value)
