...
def generate_idx_name(usr_id: str, group_id: str, agent_id: Optional[str] = None, mem_type: Optional[str] = None):
    ...
    if agent_id:
        if mem_type:
            return 'single_agent^{}^{}^{}^{}'.format(usr_id, group_id, agent_id, mem_type)
        else:
            return 'single_agent^{}^{}^{}^null'.format(usr_id, group_id, agent_id)
    if mem_type:
        return 'single_agent^{}^{}^null^{}'.format(usr_id, group_id, mem_type)
    return 'single_agent^{}^{}^null^null'.format(usr_id, group_id)
