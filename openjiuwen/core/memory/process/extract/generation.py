...
class Generator:
    ...
    async def gen_all_memory(self, **kwargs) -> list[BaseMemoryUnit]:
        ...
        memory_logger.info(
            ...
            event_type=LogEventType.MEMORY_PROCESS,
            operation="generate all memory",
            metadata={"user_id": user_id, "scope_id": scope_id, "all_memory_result": all_memory_results}
        )
        return all_memory_results
        ...
        for profile_type, profile_list in user_profile_dict.items():
            if not isinstance(profile_list, list):
                memory_logger.warning(
                    ...
                    event_type=LogEventType.MEMORY_PROCESS,
                    operation="get user profile unit",
                    metadata={"user_id": user_id, "scope_id": scope_id, "profile_list": profile_list}
                )
                continue
