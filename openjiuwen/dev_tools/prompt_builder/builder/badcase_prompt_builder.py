    ...
    def _validate_input(self, prompt: str, cases: List[EvaluatedCase]):
        if prompt is None:
            raise JiuWenBaseException(
                StatusCode.PROMPT_BUILDER_FEEDBACK_TEMPLATE_ERROR.code,
                StatusCode.PROMPT_BUILDER_FEEDBACK_TEMPLATE_ERROR.errmsg.format(
                    error_msg=f"prompt cannot be None"
                )
            )
        if not prompt.strip():
            raise JiuWenBaseException(
                StatusCode.PROMPT_BUILDER_BAD_CASE_TEMPLATE_ERROR.code,
                StatusCode.PROMPT_BUILDER_BAD_CASE_TEMPLATE_ERROR.errmsg.format(
