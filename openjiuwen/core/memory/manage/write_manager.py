...
class WriteManager:
    ...
    async def delete_mem_by_scope_id(self, scope_id: str, meta_data: list[dict[str, Any]]):
        for manager in self.managers:
            if hasattr(self.managers[manager], 'delete_by_scope_id'):
                await self.managers[manager].delete_by_scope_id(scope_id=scope_id, meta_data=meta_data)
            else:
