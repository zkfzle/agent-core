...
class MessageManager:
    ...
    async def add(self, req: MessageAddRequest) -> str:
        if req.user_id is None:
            raise build_error(
                StatusCode.MEMORY_MESSAGE_MANAGER_ADD_ERROR,
                msg="must provide user_id"
            )
        if req.group_id is None:
            raise build_error(
                StatusCode.MEMORY_MESSAGE_MANAGER_ADD_ERROR,
                msg="must provide group_id"
            )
        if req.content is None:
            raise build_error(
                StatusCode.MEMORY_MESSAGE_MANAGER_ADD_ERROR,
                msg="must provide content"
            )
    ...
    async def get(self, user_id: str = None, group_id: str = None, session_id: str = None,
        ...
        if message_len <= 0:
            raise build_error(
                StatusCode.MEMORY_MESSAGE_MANAGER_GET_ERROR,
                msg="message_len Must bigger than zero"
            )
