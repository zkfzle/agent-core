...
class SessionModelContext(ModelContext):
    ...
    def pop_messages(self, size: int = 1, with_history: bool = True) -> List[BaseMessage]:
        if size is not None and size < 0:
            raise JiuWenBaseException(
                StatusCode.CONTEXT_MESSAGE_POP_ERROR.code,
                StatusCode.CONTEXT_MESSAGE_POP_ERROR.errmsg.format(
    ...
    def get_messages(self, size: Optional[int] = None, with_history: bool = True) -> List[BaseMessage]:
        if size is not None and size < 0:
            raise JiuWenBaseException(
                StatusCode.CONTEXT_MESSAGE_GET_ERROR.code,
                StatusCode.CONTEXT_MESSAGE_GET_ERROR.errmsg.format(
    ...
    async def get_context_window(self,
        ...
        if window_size is not None and window_size <= 0:
            raise JiuWenBaseException(
                StatusCode.CONTEXT_WINDOW_GET_ERROR.code,
                StatusCode.CONTEXT_WINDOW_GET_ERROR.errmsg.format(
        ...
        if isinstance(messages, list):
            for msg in messages:
                if not isinstance(msg, BaseMessage):
                    raise JiuWenBaseException(
                        StatusCode.CONTEXT_MESSAGE_VALIDATION_ERROR.code,
                        StatusCode.CONTEXT_MESSAGE_VALIDATION_ERROR.errmsg.format(
