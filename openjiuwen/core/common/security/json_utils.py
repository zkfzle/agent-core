    ...
    def safe_json_dumps(obj, default=None, **kwargs):
        if default is None:
            try:
                ...
                ExceptionUtils.raise_exception(StatusCode.COMMON_JSON_EXECUTION_PROCESS_ERROR, f"JSON serialization type error", e)
            except ValueError as e:
                ExceptionUtils.raise_exception(StatusCode.COMMON_JSON_EXECUTION_PROCESS_ERROR, f"JSON serialization value error", e)
            except Exception as e:
                ExceptionUtils.raise_exception(StatusCode.COMMON_JSON_EXECUTION_PROCESS_ERROR, f"JSON serialization error", e)
        else:
            result = default
            try:
