...
from openjiuwen.core.controller.modules import EventHandler, EventHandlerInput, TaskExecutor, TaskManager, EventQueue
...
class StatePersistenceEventHandler(EventHandler):
    ...
    async def handle_input(self, inputs: EventHandlerInput):
        ...
        if self.round_number == 1:
            ...
            existing_tasks = await self._task_manager.get_task(
...
class MultiTaskStatePersistenceEventHandler(EventHandler):
    ...
    async def handle_input(self, inputs: EventHandlerInput):
        ...
        if self.round_number == 1:
            ...
            all_tasks = await self._task_manager.get_task(task_filter=None)
...
def build_cancellable_executor(config, ability_manager, context_engine, task_manager, event_queue):
    """Build cancelable task executor"""
    return CancellableTaskExecutor(config, ability_manager, context_engine, task_manager, event_queue)


def build_non_cancellable_executor(config, ability_manager, context_engine, task_manager, event_queue):
    """Build non-cancellable task executor"""
    return NonCancellableTaskExecutor(config, ability_manager, context_engine, task_manager, event_queue)


def build_failing_executor(config, ability_manager, context_engine, task_manager, event_queue):
    """Build failing task executor"""
    return FailingTaskExecutor(config, ability_manager, context_engine, task_manager, event_queue)
...
class TestEventHandlerTaskControl:
    ...
    async def test_pause_task_in_event_handler(self):
        ...
        tasks = await agent.controller._task_manager.get_task(task_filter=TaskFilter(task_id="pause_test_task_2"))
        task_2 = tasks[0]
        ...
        tasks = await agent.controller._task_manager.get_task(task_filter=TaskFilter(task_id="pause_test_task_3"))
        task_3 = tasks[0]
    ...
    async def test_cancel_task_in_event_handler(self):
        ...
        tasks = await agent.controller._task_manager.get_task(task_filter=TaskFilter(task_id="cancel_test_task_2"))
        task_2 = tasks[0]
        assert task_2 is not None, "The second task should exist"
    ...
    async def test_pause_non_pausable_task_in_event_handler(self):
        ...
        tasks = await agent.controller._task_manager.get_task(task_filter=TaskFilter(task_id="non_pausable_task"))
        task = tasks[0]
    ...
    async def test_cancel_non_cancellable_task_in_event_handler(self):
        ...
        tasks = await agent.controller._task_manager.get_task(task_filter=TaskFilter(task_id="non_cancellable_task"))
        task = tasks[0]
        assert task is not None, "Task should exist"
        assert task.status == TaskStatus.COMPLETED, \
    ...
    async def test_pause_then_cancel_in_event_handler(self):
        ...
        # Verify the second task is paused
        tasks = await agent.controller._task_manager.get_task(task_filter=TaskFilter(task_id="multi_op_task_2"))
        task_2 = tasks[0]
        assert task_2 is not None, "The second task should exist"
        assert task_2.status == TaskStatus.PAUSED, \
...
class TestStatePersistence:
    ...
    async def test_paused_task_state_persistence(self):
        ...
        tasks = await agent.controller._task_manager.get_task(
            task_filter=TaskFilter(task_id="persist_task_2")
        )
    ...
    async def test_multi_task_state_persistence(self):
        ...
        all_tasks_round_1 = await agent.controller._task_manager.get_task(task_filter=None)
        ...
        all_tasks_round_2 = await agent.controller._task_manager.get_task(task_filter=None)
    ...
    async def test_state_restoration_failure_fallback(self):
        ...
        paused_tasks = await agent.controller._task_manager.get_task(
            task_filter=TaskFilter(task_id="persist_task_2")
        )
        ...
        try:
            ...
            all_tasks = await agent.controller._task_manager.get_task(task_filter=None)
            ...
            persisted_task = await agent.controller._task_manager.get_task(
                task_filter=TaskFilter(task_id="persist_task_2")
            )
...
class TestLifecycleManagement:
    ...
    async def test_multiple_stream_calls_no_duplicate_start(self):
        """Test multiple stream() calls on the same event loop do not restart components

        Test goals:
        1. Start EventQueue and TaskScheduler on the first stream() call
        2. On the second stream() call, do not restart them (reuse existing components)
        3. Verify event_loop tracking is correct
        """
        agent = await build_test_agent(
        ...
        assert len(output_1) > 0, "The first stream should produce output"

        # Record the event_loop after first start
        first_event_loop = agent.controller._event_loop
        assert first_event_loop is not None, "event_loop should be recorded after the first stream"

        # Second stream (same event loop)
        input_event_2 = InputEvent(
            event_type=EventType.INPUT,
        ...
        assert len(output_2) > 0, "The second stream should produce output"

        # Verify event_loop does not change (components are reused)
        second_event_loop = agent.controller._event_loop
        assert second_event_loop is first_event_loop, \
            "Multiple stream calls on the same event loop should reuse components without restarting"
    ...
    async def test_controller_stop_cleanup_all(self):
        ...
        assert agent.controller._task_scheduler._running is False, "Scheduler running state should be False"
...
class TestSessionManagement:
    ...
    async def test_multi_turn_conversation_no_interference(self):
        ...
        tasks_after_turn_1 = await agent.controller._task_manager.get_task(task_filter=None)
        ...
        tasks_after_turn_2 = await agent.controller._task_manager.get_task(task_filter=None)
...
class TestEventSystem:
    ...
    async def test_event_unsubscribe_cleanup(self):
        """Test event unsubscription and cleanup

        Test goals:
        1. Correctly unsubscribe when stream() ends
        2. Verify subscriptions do not leak

        Verification strategy:
        - Each stream() call creates subscriptions for 4 event types
        - After stream() ends, all subscriptions should be cleaned up
        - After multiple stream() calls, subscription count should remain stable (no accumulation)
        """
        agent = await build_test_agent(
            agent_id="test_unsub",
            event_handler=DynamicTaskEventHandler(),
        ...
        session = TaskSession(session_id="test_unsub")

        # Get MessageQueue._subscribers (used to verify subscription cleanup)
        message_queue = agent.controller.event_queue._queue

        # Before first stream, subscription count should be 0
        initial_sub_count = len(message_queue._subscribers)
        logger.info(f"Initial subscription count: {initial_sub_count}")
        assert initial_sub_count == 0, f"Initial subscription count should be 0, but is {initial_sub_count}"
        ...
        # After first stream ends, verify subscriptions are cleaned up
        sub_count_after_first = len(message_queue._subscribers)
        logger.info(f"Subscription count after first stream: {sub_count_after_first}")
        assert sub_count_after_first == 0, \
            f"Subscriptions should be cleaned after first stream, expected 0, got {sub_count_after_first}"

        # Second stream (verify no cumulative leaks)
        input_event_2 = InputEvent(
            event_type=EventType.INPUT,
        ...
        # After second stream ends, verify subscriptions are still cleaned up
        sub_count_after_second = len(message_queue._subscribers)
        logger.info(f"Subscription count after second stream: {sub_count_after_second}")
        assert sub_count_after_second == 0, \
            f"Subscriptions should be cleaned after second stream, expected 0, got {sub_count_after_second}"

        logger.info("✅ test_event_unsubscribe_cleanup passed")
