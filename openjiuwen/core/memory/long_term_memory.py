...
class LongTermMemory(metaclass=Singleton):
        ...
        try:
            ...
            if self.kv_store:
                try:
                    ...
                    logger.error(f"Failed to delete user memories for scope {scope_id}", exc_info=e)
    ...
    async def add_messages(
        ...
        msg_id = "-1"
        llm = await self._get_scope_llm_and_embedding(scope_id)
    ...
    async def get_user_mem_by_page(self,
                                   ...
                                   page_size: int = 10,
                                   page_idx: int = 0,
                                   type: MemoryType = MemoryType.UNKNOWN) -> list[MemInfo]:
        """
        ...
        Args:
            ...
            type: Memory type to filter. If UNKNOWN, no filtering is applied.
        ...
        for item in search_data:
            ...
            if type == MemoryType.UNKNOWN or mem_type == type:
                mem_results.append(
                    MemInfo(
                        mem_id=item["id"],
                        content=item["mem"],
    ...
    async def _get_scope_llm_and_embedding(self, scope_id: str) -> Tuple[str, Model]:
        """
        ...
        try:
            ...
            if config and config.model_cfg and config.model_client_cfg:
                ...
                # 如果scope配置不存在或LLM配置不完整，使用base llm
                llm = self._base_llm

            # 如果LLM获取失败，尝试使用系统默认配置
            if (not llm and self._sys_mem_config and self._sys_mem_config.default_model_client_cfg
                    and self._sys_mem_config.default_model_cfg):
                try:
