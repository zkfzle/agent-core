...
from openjiuwen.core.foundation.llm1 import BaseMessage, AssistantMessage, UserMessage
...
class MessageOffloaderConfig(BaseModel):
    ...
    """Reserved flag for future use; currently unused by the offloader."""
...
class MessageOffloader(ContextProcessor):
    def __init__(self, config: MessageOffloaderConfig):
        ...
        if config.trim_size >= config.large_message_threshold:
            raise ValueError()
        if (
            config.messages_to_keep
            and config.messages_threshold
            and config.messages_to_keep >= config.messages_threshold
        ):
            raise ValueError()
    ...
    async def on_add_messages(self,
        ...
        processed_messages = await self._offload_large_messages(context_messages)
    ...
    async def trigger_add_messages(
        ...
        # 不处理长度小于messages_to_keep的上下文
        if config.messages_to_keep and message_size <= config.messages_to_keep:
            return False

        # 处理长度超过messages_threshold的上下文
        if config.messages_threshold and message_size > config.messages_threshold:
            logger.info(f"[MessageOffloader triggered] context messages num {message_size} exceeds threshold of {config.messages_threshold}")
            return True
        ...
        if tokens > config.tokens_threshold:
            logger.info(f"[MessageOffloader triggered] context tokens {tokens} exceeds threshold of {config.tokens_threshold}")
            return True
        return False

    async def _offload_large_messages(self, messages: List[BaseMessage]) -> List[BaseMessage]:
    ...
    async def _offload_message(self, message: BaseMessage) -> OffloadMessage:
        trimmed_content = "[TRIM-OFFLOAD]" + message.content[:self.config.trim_size] + OMIT_STRING
