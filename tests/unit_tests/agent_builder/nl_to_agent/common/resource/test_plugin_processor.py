...
class TestContextManager(unittest.TestCase):
    ...
    def test_preprocess(self):
        plugin_dict, tool_plugin_id_map = PluginProcessor.preprocess(self.raw_plugins)
        self.assertIn("mock_plugin_1", plugin_dict)
        self.assertIn("mock_tool_1", plugin_dict["mock_plugin_1"]["tools"])
        self.assertEqual(
            self.raw_plugins[0]["tools"][0]["input_parameters"][0],
    ...
    def test_get_retrieved_info(self):
        ...
        result = PluginProcessor.get_retrieved_info(tool_id_list, self.plugin_dict, self.tool_plugin_id_map, need_inputs_outputs=True)
        expected_tools = [
            {
                "tool_id": "mock_tool_1",
                "tool_name": "Mock Tool One",
        ...
        result_no_io = PluginProcessor.get_retrieved_info(tool_id_list, self.plugin_dict, self.tool_plugin_id_map, need_inputs_outputs=False)
        expected_tools_no_io = [
            {
                "tool_id": "mock_tool_1",
                "tool_name": "Mock Tool One",
                "tool_desc": "Description of Mock Tool One",
        ...
        self.assertEqual(result_no_io, expected_tools_no_io)

    def test_collect_plugin(self):
        tool_id_list = ["mock_tool_1", "mock_tool_3"]
        result = PluginProcessor.collect_plugin(tool_id_list, self.plugin_dict, self.tool_plugin_id_map)
        expected = [
            {
                "plugin_id": "mock_plugin_1",
                "plugin_name": "Mock Plugin One",
