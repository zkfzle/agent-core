...
from openjiuwen.core.foundation.llm1 import (
    ...
    ModelConfig, ModelClientConfig, Model
)
...
class MessageSummaryOffloaderConfig(BaseModel):
    ...
    **Evaluation order (highest → lowest priority):**
    ...
    """Token length above which a single message is labelled *large* and becomes a **preferred** compression candidate."""
    ...
    """When *True*, the inference backend is **asked** to drop KV cache of off-loaded messages → **GPU memory↓**, re-generation speed↓."""
    ...
    """Optional **client-level** settings (endpoint, timeout, retry, headers) for the model that **performs** the summary/compression."""
...
"""
You are a "high-density summarizer". Your task is to shrink the overly long message below into 2–4 concise sentences that:
...
class MessageSummaryOffloader(MessageOffloader):
    def __init__(self, config: MessageSummaryOffloaderConfig):
        ContextProcessor.__init__(self, config)
        if (
    ...
    async def _offload_message(self, message: BaseMessage) -> OffloadMessage:
        ...
        summarized_content = "[SUMMARY-OFFLOAD]" + response.content
        offload_message = MemoryOffloadMessage(role=message.role, content=summarized_content)
        original_content = message.content.replace('\n', '\\n')
        print(f"before: {original_content}\nafter: {summarized_content}")
