...
class CancellableTaskExecutor(TaskExecutor):
    ...
    async def execute_ability(self, task_id: str, session: Session) -> AsyncIterator[ControllerOutputChunk]:
        ...
        # Decide execution time based on task ID: task_1 finishes quickly (2 iterations), task_2 and task_3 run slowly (100 iterations)
...
class PauseInHandlerEventHandler(EventHandler):
    ...
    async def handle_task_completion(self, inputs: EventHandlerInput):
        ...
        # When the third task completes, cancelling the paused second task fails because task_2 is paused and not in _running_tasks
...
class StatePersistenceEventHandler(EventHandler):
    ...
    async def handle_input(self, inputs: EventHandlerInput):
        ...
        if self.round_number == 1:
            ...
            if existing_tasks:
                logger.info(
                    f"StatePersistenceEventHandler: ✅ Found persisted task {existing_tasks[0].task_id} in second round, status: {existing_tasks[0].status}")
            else:
...
class MultiTaskStatePersistenceEventHandler(EventHandler):
    ...
    async def handle_input(self, inputs: EventHandlerInput):
        ...
        if self.round_number == 1:
            ...
            all_tasks = self._task_manager.get_task(task_filter=None)
...
class TestStatePersistence:
    ...
    async def test_multi_task_state_persistence(self):
        ...
        all_tasks_round_1 = agent.controller._task_manager.get_task(task_filter=None)
        ...
        all_tasks_round_2 = agent.controller._task_manager.get_task(task_filter=None)
        ...
        assert len(all_tasks_round_2) == 3, f"The second round should restore 3 tasks, but restored {len(all_tasks_round_2)}"
        assert status_map_2 == status_map_1, \
            f"States in the second round should match the first round\nFirst round: {status_map_1}\nSecond round: {status_map_2}"
    ...
    async def test_state_restoration_failure_fallback(self):
        ...
        try:
            ...
            # Note: the second round will create new tasks (because round_number = 2), but persist_task_2 should not be found
...
class TestEventSystem:
    ...
    async def test_event_unsubscribe_cleanup(self):
        ...
        - Each stream() call creates subscriptions for 4 event types (INPUT, TASK_INTERACTION, TASK_COMPLETION, TASK_FAILED)
