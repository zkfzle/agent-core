            ...
            if config.index_type in ("vector", "hybrid"):
                if not embed_model:
                    raise JiuWenBaseException(
                        ...
                        StatusCode.RETRIEVAL_INDEXING_EMBED_MODEL_NOT_FOUND.errmsg.format(error_msg="embed_model is required for vector/hybrid index type"),
                    )
        ...
        if config.index_type in ("vector", "hybrid"):
            ...
            if dimension is None or dimension == 0:
                raise JiuWenBaseException(
                    ...
                    StatusCode.RETRIEVAL_INDEXING_DIMENSION_NOT_FOUND.errmsg.format(error_msg="dimension is required for vector/hybrid index type"),
                )
