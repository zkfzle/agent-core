    ...
    def _validate_input(self, prompt: str, cases: List[EvaluatedCase]):
        if prompt is None:
            raise JiuWenBaseException(
                StatusCode.TOOLCHAIN_FEEDBACK_TEMPLATE_ERROR.code,
                StatusCode.TOOLCHAIN_FEEDBACK_TEMPLATE_ERROR.errmsg.format(
                    error_msg=f"prompt cannot be None"
                )
            )
        if not prompt.strip():
            raise JiuWenBaseException(
                StatusCode.TOOLCHAIN_BAD_CASE_TEMPLATE_ERROR.code,
                StatusCode.TOOLCHAIN_BAD_CASE_TEMPLATE_ERROR.errmsg.format(
