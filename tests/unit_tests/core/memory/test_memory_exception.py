...
async def test_register_store_kv_store_none():
    ...
    err = e.value
    assert err.status == StatusCode.MEMORY_STORE_REGISTER_FAILED
    assert err.code == StatusCode.MEMORY_STORE_REGISTER_FAILED.code
...
async def test_register_store_semantic_store_wrong_type():
    ...
    assert e.value.status == StatusCode.MEMORY_STORE_REGISTER_FAILED
...
def test_set_scope_config_llm_init_failed():
    ...
    err = e.value
    assert err.status == StatusCode.MEMORY_SET_CONFIG_OPERATION_FAILED
...
async def test_delete_mem_by_id_write_manager_not_init():
    ...
    err = e.value
    assert err.status == StatusCode.MEMORY_DELETE_MEMORY_OPERATION_FAILED
