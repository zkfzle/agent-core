...
class TestAPIEmbedding:
    ...
    def test_init_with_api_key(self, embedding_config):
        ...
        assert "Authorization" in model._headers
        assert model._headers["Authorization"] == "Bearer test-api-key"

    def test_init_without_api_key(self, embedding_config_no_key):
        """Test initialization without API key"""
        model = APIEmbedding(config=embedding_config_no_key)
        assert model.api_key is None
        assert "Authorization" not in model._headers

    def test_init_with_extra_headers(self, embedding_config):
        """Test initialization with extra headers"""
        extra_headers = {"X-Custom-Header": "custom-value"}
        model = APIEmbedding(config=embedding_config, extra_headers=extra_headers)
        assert model._headers["X-Custom-Header"] == "custom-value"

    def test_init_with_custom_params(self, embedding_config):
    ...
    @pytest.mark.asyncio
    async def test_get_embeddings_with_list_input(self, embedding_config):
        """Test getting embeddings with list input"""
        mock_response = Mock()
        mock_response.json.return_value = {
        ...
        with patch("asyncio.to_thread") as mock_to_thread:
            ...
            model = APIEmbedding(config=embedding_config)
            embeddings = await model._get_embeddings(["text 1", "text 2"])
            assert len(embeddings) == 2
            assert all(len(emb) == 384 for emb in embeddings)

    @pytest.mark.asyncio
    async def test_get_embeddings_with_string_input(self, embedding_config):
        """Test getting embeddings with string input"""
        mock_response = Mock()
        mock_response.json.return_value = {"embedding": [0.1] * 384}
        mock_response.raise_for_status = Mock()
        ...
        with patch("asyncio.to_thread") as mock_to_thread:
            ...
            model = APIEmbedding(config=embedding_config)
            embeddings = await model._get_embeddings("text")
