...
class WhitespaceNormalizer(TextPreprocessor):
    ...
    def __init__(self, preserve_single_newline: bool = False):
        """
        Initialize the whitespace normalizer.

        Args:
            preserve_single_newline: If True, preserves single newlines but
                                     removes multiple consecutive newlines
        """
        self.preserve_single_newline = preserve_single_newline
    ...
    def process(self, text: str) -> str:
        ...
        if self.preserve_single_newline:
            # Replace multiple newlines with single newline
            text = re.sub(r"\n{2,}", "\n", text)
            # Replace consecutive spaces and tabs with single space
            text = re.sub(r"[ \t]+", " ", text)
            # Replace space before/after newline
            text = re.sub(r" *\n *", "\n", text)
        else:
