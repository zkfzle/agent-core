...
class StatusCode(Enum):
    ...
    CONTEXT_OPERATION_ERROR = {
        "add": (153001, "context add message error, reason: {error_msg}"),
        "get": (153002, "context get message error, reason: {error_msg}"),
        "pop": (153003, "context pop message error, reason: {error_msg}"),
        "get context window": (153004, "context get context window error, reason: {error_msg}")
    }
    CONTEXT_PROCESS_ERROR = (153000, "context process message error, reason: {error_msg}")
    CONTEXT_VALIDATION_ERROR = (153005, "context engine message validation error, reason: {error_msg}")
    ...
    TOOLCHAIN_AGENT_PARAMS_ERROR = (170000, "parameters error, reason: {error_msg}")
    TOOLCHAIN_OPTIMIZER_BACKWORD_ERROR = (170010, "do optimizer's backward failed, reason: {error_msg}")
    TOOLCHAIN_OPTIMIZER_UPDATE_ERROR = (170011, "do optimizer's update failed, reason: {error_msg}")
    TOOLCHAIN_OPTIMIZER_PARAMS_ERROR = (170012, "agent optimizer parameters error, reason: {error_msg}")
    TOOLCHAIN_EVALUATOR_EVALUATE_ERROR = (170030, "do evaluator's evaluate failed, reason: {error_msg}")
    TOOLCHAIN_TRAINER_TRAIN_ERROR = (170040, "do trainer's train error, reason: {error_msg}")
    ...
    TOOLCHAIN_META_TEMPLATE_REGISTER_ERROR = (173000, "meta-template register failed, reason: {error_msg}")
    TOOLCHAIN_META_TEMPLATE_ERROR = (173001, "meta-template builder  failed, reason: {error_msg}")
    TOOLCHAIN_FEEDBACK_TEMPLATE_ERROR = (173002, "feedback-template builder failed, reason: {error_msg}")
    TOOLCHAIN_BAD_CASE_TEMPLATE_ERROR = (173003, "bad case-template builder failed, reason: {error_msg}")
    # Common Capabilities 180000 - 189999
    # Common Capabilities - Prompt Template 180000 - 180999
    PROMPT_FORMAT_ERROR = (180000, "prompt template format error, reason: {error_msg}")
    PROMPT_INVALID_CONTENT_ERROR = (180001, "prompt template data content type invalid)")
