...
class TestChromaVectorStore:
    ...
    async def test_remove_collection_from_cache(self, chroma_store, persist_directory, table_name):
        ...
        # 从缓存中移除集合
        await chroma_store.remove_collection_from_cache(table_name)
        assert table_name not in chroma_store.collection_cache

    @pytest.mark.asyncio
    async def test_check_table_name(self, chroma_store):
        """测试检查表名"""
        # 测试空表名应该抛出异常
        with pytest.raises(ValueError, match="Chroma collection name is required for test_operation"):
            await chroma_store.check_table_name(None, "test_operation")

        with pytest.raises(ValueError, match="Chroma collection name is required for test_operation"):
            await chroma_store.check_table_name("", "test_operation")
