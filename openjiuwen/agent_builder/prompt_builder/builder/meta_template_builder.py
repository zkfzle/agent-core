...
import os
import re
from typing import List, Optional, Generator, Literal, Callable
...
from openjiuwen.core.utils.prompt.index.template_store.template_store import TemplateStore
...
class MetaTemplateBuilder(BasePromptBuilder):
    ...
    @classmethod
    def register_meta_template(cls, name: str, meta_template: str | Template):
        ...
        TemplateManager().register(template_to_reg, force=True)
    ...
    @staticmethod
    def _format_custom_meta_template(custom_meta_template_name: str,
                                     prompt: str | Template,
                                     tools: Optional[List[ToolInfo]] = None
                                     ):
        custom_meta_template_name = f"{META_TEMPLATE_NAME_PREFIX}{custom_meta_template_name}"
        custom_meta_template = TemplateManager().get(name=custom_meta_template_name)
