...
class LoopGroup(BaseWorkflow, Executable):
    ...
    def add_workflow_comp(
        ...
        if isinstance(workflow_comp, LoopComponent):
            raise JiuWenBaseException(StatusCode.COMPONENT_LOOP_EXECUTION_NOT_SUPPORT.code,
                                      StatusCode.COMPONENT_LOOP_EXECUTION_NOT_SUPPORT.errmsg)
    ...
    def _atomic_invoke(self, **kwargs) -> Any:
        try:
            ...
            if isinstance(e, JiuWenBaseException):
                raise
            raise JiuWenBaseException(StatusCode.COMPONENT_EXECUTION_ERROR.code,
                                      StatusCode.COMPONENT_EXECUTION_ERROR.errmsg.format(error_msg=str(e))) from e
...
class LoopComponent(WorkflowComponent):
    def __init__(self, loop_group: LoopGroup, output_schema: dict):
        ...
        if loop_group.is_empty:
            raise JiuWenBaseException(StatusCode.COMPONENT_LOOP_EXECUTION_NOT_FOUND.code,
    ...
    async def invoke(self, inputs: Input, session: Session, context: ModelContext) -> Output:
        try:
            ...
            if loop_input.loop_type == LoopType.Array.value:
                ...
                raise JiuWenBaseException(StatusCode.COMPONENT_LOOP_EXECUTION_CONFIG_ERROR.code,
