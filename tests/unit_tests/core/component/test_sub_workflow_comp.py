...
class TestSubWorkflowComp:
    ...
    async def test_sub_invoke(self):
        with pytest.raises(JiuWenBaseException) as err:
            ...
            await main_workflow.invoke(inputs={}, session=WorkflowSession())
        sub_workflow_code = StatusCode.COMPONENT_SUB_WORKFLOW_RUNTIME_ERROR.code
        ...
        assert err.value.message == StatusCode.WORKFLOW_COMPONENT_RUNTIME_ERROR.errmsg.format(
            error_msg = f"node_id: sub2, ability: invoke, error: [{sub_workflow_code}] {sub_workflow_err}"
        )
