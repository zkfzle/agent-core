...
from openjiuwen.core.memory.manage.scope_user_manager import ScopeUserManager
...
class LongTermMemory(metaclass=Singleton):
    ...
    def set_config(self, config: MemoryEngineConfig):
        ...
        if self.db_store:
            ...
            self.scope_user_manager = ScopeUserManager(sql_db_store)
    ...
    async def delete_mem_by_scope(self, scope_id: str) -> bool:
        ...
        if not self._validate_id(scope_id=scope_id):
            logger.error(f"Invalid scope_id format, scope_id={scope_id}")
            return False
        lock = DistributedLock(self.kv_store, f"scope/{scope_id}")
        async with lock:
            scope_user_data = await self.scope_user_manager.get_by_scope_id(scope_id=scope_id)
            ...
            await self.scope_user_manager.delete_by_scope_id(scope_id=scope_id)
    ...
    async def add_messages(
        ...
        async with lock:
            ...
            # add meta data
            await self.scope_user_manager.add(user_id=user_id, scope_id=scope_id)
