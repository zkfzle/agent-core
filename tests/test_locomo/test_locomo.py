...
class RequestRerankerModel(RequestChatModel):
    ...
    @abstractmethod
    def _build_request_input_parameters(
        self, model_name: str, query: str, documents: List[Dict], **kwargs
    ):
    ...
    def _request_params(
        ...
        **kwargs: Any,
        ...
        params = self._build_request_input_parameters(
            model_name=model_name, query=query, documents=documents, **kwargs
        )
        ...
        if UserConfig.is_sensitive():
            ...
            logger.info(
                f"Before request chat model, request params is ready. "
...
class ContextFilter:
    ...
    async def afilter(self, query: str, documents: list[dict]):
        ...
        scores = await self.reranker.arerank(
            query=query, documents=[d["mem"] for d in documents]
        )
        document_scores = sorted(
...
class ConversationProcessor:
        ...
        async with self.init_lock:
            ...
            with self.global_chroma_lock:
                ...
                self.resource_dir = os.path.join(
                    project_root, f"resources_conv_{self.conv_id}"
                )
                os.makedirs(self.resource_dir, exist_ok=True)
                logger.info(
                    f"📁 Conversation {self.conv_id} 资源目录: {self.resource_dir}"
                )
                ...
                db_path = (
                    db_path
                    or Path(
                        f"{self.resource_dir}/test_sql_db_{time_str}_{uuid_str}_{self.conv_id}.db"
                    ).resolve()
                )
                ...
                self.memory_engine = MemoryEngine(
                    SysMemConfig(), kv_store, semantic_store, db_store
                )
                ...
                self.llm_base = ModelFactory().get_model(
                    MODEL_PROVIDER, API_KEY, API_BASE, temperature=TEMPERATURE or 0.05
                )

                self.init_done = True
                logger.info(
                    f"✅ Conversation {self.conv_id} 资源初始化完成（独立MemoryEngine）"
                )
    ...
    async def add_memory(
        ...
        for retry in range(retries):
            try:
                ...
                logger.debug(
                    f"📝 Conversation {self.conv_id} 成功添加 {len(messages)} 条内存"
                )
                break
                ...
                if retry < retries - 1:
                    ...
                    logger.warning(
                        f"⚠️ Conversation {self.conv_id} 添加内存失败，重试 {retry+1}/3: {str(e)}"
                    )
                    continue
                else:
                    logger.error(
                        f"❌ Conversation {self.conv_id} 添加内存失败: {str(e)}"
                    )
                    raise e
            ...
            for chat in chats:
                ...
                if len(messages) == 4:
                    await self.add_memory(
                        user_id, app_id, messages, timestamp, session_id
                    )
                    messages = []
    ...
    async def llm_answer(
        self, user_id: str, app_id: str, query: str, retrieve_num: int = 50
    ) -> str:
        """异步调用LLM生成回答（读取私有目录数据）"""
        # 使用私有memory_engine检索
        user_memory = await self.memory_engine.search_user_mem(
            user_id=user_id, group_id=app_id, query=query, num=retrieve_num
        )
        if user_memory:
            filtered_memory = await self.context_filter.afilter(
                query=query, documents=user_memory
            )
        ...
        memory_msg_list = [
            m for _, m in sorted(memory_msg_list, key=lambda x: x[0], reverse=False)
        ]
        ...
        logger.debug(
            f"📝 Conversation {self.conv_id} LLM Prompt: {llm_prompt[:100]}..."
        )

        message = HumanMessage(content=llm_prompt)
        response = await self.llm_base.ainvoke(
            model_name=MODEL_NAME, messages=[message]
        )
            ...
            for retry in range(3):
                try:
                    ...
                    if retry < 2:
                        await asyncio.sleep(30)
                        logger.warning(
                            f"⚠️ Conversation {self.conv_id} QA生成失败，重试 {retry+1}/3: {str(e)}"
                        )
                        continue
                    else:
                        logger.error(
                            f"❌ Conversation {self.conv_id} QA生成失败: {str(e)}"
                        )
                        raise e
    ...
    async def llm_service(self, user_message: str) -> AIMessage:
        ...
        messages = [
            ...
            BaseMessage(**{"role": "user", "content": user_message}),
        ]
        for retry in range(3):
            try:
                ...
                if retry < 2:
                    await asyncio.sleep(30)
                    logger.warning(
                        f"⚠️ Conversation {self.conv_id} 判断失败，重试 {retry+1}/3: {str(e)}"
                    )
                    continue
                else:
    ...
    async def validate_locomo_data(self, speaker_a: str, speaker_b: str):
        ...
        with open(response_path_enum, "r", encoding="utf-8") as f:
            for qa_enum_str in f:
                ...
                logger.info(
                    f"🔍 Conversation {self.conv_id} 验证结果: {result.content[:50]}..."
                )
    ...
    async def process_full(
        self, data_enum: dict, db_path: str = None, build_memory=True
    ):
        ...
        try:
            ...
            logger.error(
                f"❌ Conversation {self.conv_id} 处理失败: {str(e)}", exc_info=True
            )
            raise
        finally:
            # 更新进度
...
class TESTLOCOMO:
    ...
    def overall_compute(result_path_enum: str) -> None:
        ...
        total_accuracy["overall"] = round(
            total_correct_all / total_samples if total_samples != 0 else 0, 4
        )
...
async def main(args):
    ...
    for conv_id in args.conversation_ids:
        ...
        # 添加到任务列表
        task = processor.process_full(
            data_enum, db_path=args.db_path, build_memory=args.build_memory
        )
        tasks.append(task)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--conversation-ids", type=int, nargs="+", default=list(range(10))
    )
