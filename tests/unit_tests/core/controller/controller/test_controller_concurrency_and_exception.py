...
class TestConcurrentSessionIsolation:
    ...
    async def test_session_task_isolation(self):
        ...
        failed_tasks = [t for t in await agent_2.controller._task_manager.get_task(task_filter=None)
                       if t.status == TaskStatus.FAILED]
        assert len(failed_tasks) > 0, "Session 2 should have failed tasks"

        # Verify Session 1 has no failed tasks
        all_tasks_1 = await agent_1.controller._task_manager.get_task(task_filter=None)
...
class TestExceptionHandling:
    ...
    async def test_task_execution_exception_handling(self):
        ...
        failed_tasks = await agent.controller._task_manager.get_task(
        ...
        normal_tasks = await agent.controller._task_manager.get_task(
    ...
    async def test_stream_output_exception_isolation(self):
        ...
        stream_fail_tasks = await agent.controller._task_manager.get_task(
...
class TestControllerConfig:
    ...
    async def test_task_timeout(self):
        ...
        agent = await build_test_agent(
            ...
            task_executors={
                "slow": lambda cfg, am, ce, tm, eq: build_slow_executor(
                    cfg, am, ce, tm, eq, sleep_time=10.0
                )
            }
        )
