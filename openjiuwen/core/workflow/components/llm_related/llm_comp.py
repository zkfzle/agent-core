...
class LLMExecutable(ComponentExecutable):
    ...
    def _validate_template_content(template_content):
        if len(template_content) >= 1:
            try:
                ...
                raise build_error(StatusCode.COMPONENT_LLM_TEMPLATE_CONFIG_ERROR, error_msg="system message is invalid", cause=e)
            ...
            for element in template_content:
                ...
                if if_contain_user_message and element.get(_ROLE, "") == "system":
                    SystemMessage.model_validate(element)
                    raise build_error(StatusCode.COMPONENT_LLM_TEMPLATE_CONFIG_ERROR, error_msg="system message must be before user message")
            if not if_contain_user_message:
                raise build_error(StatusCode.COMPONENT_LLM_TEMPLATE_CONFIG_ERROR, error_msg="user message is required")
        else:
            raise build_error(StatusCode.COMPONENT_LLM_TEMPLATE_CONFIG_ERROR, error_msg="template content is empty")

    @staticmethod
    def _validate_output_config(output_config):
        if not output_config:
            raise build_error(StatusCode.COMPONENT_LLM_CONFIG_ERROR, error_msg="output config is empty")
        for param, value in output_config.items():
            if not param:
                raise build_error(StatusCode.COMPONENT_LLM_CONFIG_ERROR,
            ...
            try:
                ...
                if UserConfig.is_sensitive():
                    raise build_error(StatusCode.COMPONENT_LLM_CONFIG_ERROR,
                                         error_msg="output config parameter's config value is invalid")
                else:
                    raise build_error(StatusCode.COMPONENT_LLM_CONFIG_ERROR,
                                      error_msg=f"output config parameter's config {value} is invalid", cause=e)
    ...
    def _validate_response_format(response_format, output_config):
        ...
        try:
            ...
            raise build_error(StatusCode.COMPONENT_LLM_CONFIG_INVALID, error_msg=f"response format {response_format} is invalid", cause=e)

        if response_type in ["text", "markdown"] and len(output_config) != 1:
            raise build_error(StatusCode.COMPONENT_LLM_CONFIG_INVALID,
    ...
    async def invoke(self, inputs: Input, session: Session, context: ModelContext) -> Output:
        ...
        try:
            ...
            if UserConfig.is_sensitive():
                raise build_error(StatusCode.COMPONENT_LLM_INVOKE_CALL_FAILED,
                                               error_msg="invoke llm failed", cause=e)
            else:
                raise build_error(StatusCode.COMPONENT_LLM_INVOKE_CALL_FAILED, error_msg=str(e), cause=e)
    ...
    async def stream(self, inputs: Input, session: Session, context: ModelContext) -> AsyncIterator[Output]:
        ...
        try:
            ...
            if UserConfig.is_sensitive():
                raise build_error(StatusCode.COMPONENT_LLM_INVOKE_CALL_FAILED,
                                  error_msg="Failed to stream", cause=e)
            else:
                raise build_error(StatusCode.COMPONENT_LLM_INVOKE_CALL_FAILED,
                                  error_msg=str(e), cause=e)
    ...
    def _initialize_if_needed(self):
        if not self._initialized:
            try:
                ...
                raise build_error(StatusCode.COMPONENT_LLM_INIT_FAILED,
                                               error_msg="Failed to initialize llm if needed", cause=e)
    ...
    def _create_output(self, llm_output) -> Output:
        try:
            ...
            if e.error_code == StatusCode.COMPONENT_LLM_TEMPLATE_INVALID.code:
                raise build_error(StatusCode.COMPONENT_LLM_EXECUTION_PROCESS_ERROR, error_msg=e.message)
            else:
                raise e
