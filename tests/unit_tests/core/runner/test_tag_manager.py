...
class TestTagMgr:
    ...
    def setup_method(self):
        ...
        self.tag_mgr = TagMgr()
        # 初始化一些测试资源
        self.tag_mgr._resource_tags = {
            "res1": {"tag1", "tag2"},
            "res2": {"tag2", "tag3"},
            "res3": {GLOBAL},
            "res4": {"tag1", "tag3", "tag4"}
        }
        self.tag_mgr._tag_to_resource = {
            "tag1": {"res1", "res4"},
            "tag2": {"res1", "res2"},
            "tag3": {"res2", "res4"},
            "tag4": {"res4"},
    ...
    def test_has_tag(self):
        """测试 has_tag 方法"""
        assert self.tag_mgr.has_tag("tag1") is True
        assert self.tag_mgr.has_tag("tag5") is False
        assert self.tag_mgr.has_tag(GLOBAL) is True

    def test_list_tags(self):
        """测试 list_tags 方法"""
        tags = self.tag_mgr.list_tags()
        assert "tag1" in tags
        assert "tag2" in tags
        assert "tag3" in tags
        assert "tag4" in tags
    ...
    def test_has_resource(self):
        """测试 has_resource 方法"""
        assert self.tag_mgr.has_resource("res1") is True
        assert self.tag_mgr.has_resource("res5") is False

    def test_tag_resource_normal(self):
        """测试正常添加标签"""
        # 为 res1 添加新标签
        current_tags = self.tag_mgr.tag_resource("res1", ["tag5", "tag6"])
        ...
        # 验证标签到资源的映射
        assert "res1" in self.tag_mgr._tag_to_resource["tag5"]
        assert "res1" in self.tag_mgr._tag_to_resource["tag6"]

    def test_tag_resource_with_global(self):
        """测试添加 GLOBAL 标签"""
        current_tags = self.tag_mgr.tag_resource("res1", GLOBAL)
        assert current_tags == [GLOBAL]
        assert self.tag_mgr._resource_tags["res1"] == {GLOBAL}
        assert "res1" in self.tag_mgr._tag_to_resource[GLOBAL]

    def test_tag_resource_not_exist(self):
        """测试为不存在的资源添加标签"""
        with pytest.raises(Exception) as exc_info:
            self.tag_mgr.tag_resource("res99", "tag1")
        assert "Resource does not exist" in str(exc_info.value)

    def test_remove_resource(self):
        """测试删除资源"""
        removed_tags = self.tag_mgr.remove_resource("res1")
        assert set(removed_tags) == {"tag1", "tag2"}
        assert not self.tag_mgr.has_resource("res1")

        # 验证标签映射也被清理
        assert "res1" not in self.tag_mgr._tag_to_resource["tag1"]
        assert "res1" not in self.tag_mgr._tag_to_resource["tag2"]

    def test_remove_nonexistent_resource(self):
        """测试删除不存在的资源"""
        result = self.tag_mgr.remove_resource("res99")
        assert result == []

    def test_remove_resource_tags(self):
        """测试删除指定标签"""
        remaining_tags = self.tag_mgr.remove_resource_tags("res1", ["tag1", "tag3"])
    ...
    def test_update_resource_tags_replace(self):
        """测试替换标签策略"""
        new_tags = ["tag5", "tag6"]
        current_tags = self.tag_mgr.update_resource_tags(
    ...
    def test_update_to_global(self):
        """测试更新为 GLOBAL 标签"""
        old_tags = self.tag_mgr.update_resource_tags(
    ...
    def test_remove_tag(self):
        """测试删除标签"""
        affected_resources = self.tag_mgr.remove_tag("tag1")
        assert "res1" in affected_resources
        assert "res4" in affected_resources
        assert not self.tag_mgr.has_tag("tag1")

        # 验证资源标签被清理
        assert "tag1" not in self.tag_mgr._resource_tags["res1"]
        assert "tag1" not in self.tag_mgr._resource_tags["res4"]

    def test_get_tag_resources(self):
        """测试获取标签对应的资源"""
        resources = self.tag_mgr.get_tag_resources("tag1")
    ...
    def test_find_resources_by_tags_any(self):
        """测试 ANY 匹配策略"""
        resources = self.tag_mgr.find_resources_by_tags(
    ...
    def test_find_resources_with_nonexistent_tag(self):
        """测试查找不存在的标签"""
        with pytest.raises(Exception) as exc_info:
            self.tag_mgr.find_resources_by_tags(
    ...
    def test_has_resource_tag(self):
        """测试检查资源是否有指定标签"""
        assert self.tag_mgr.has_resource_tag("res1", "tag1") is True
        assert self.tag_mgr.has_resource_tag("res1", "tag3") is False
        assert self.tag_mgr.has_resource_tag("res3", GLOBAL) is True

    def test_get_resources_tags(self):
        """测试获取资源的标签"""
        tags = self.tag_mgr.get_resources_tags("res1")
        assert set(tags) == {"tag1", "tag2"}

    def test_display(self):
        """测试显示状态"""
        result = self.tag_mgr.display(enable_log=False)
    ...
    def test_concurrent_operations(self):
        ...
        def add_tags(resource_id, tags):
            try:
                self.tag_mgr.tag_resource(resource_id, tags)
            except Exception:
                pass
    ...
    def test_normalize_tags(self):
        """测试标签标准化"""
        # 测试单个标签
        result = self.tag_mgr._normalize_tags("tag1")
        assert result == {"tag1"}

        # 测试标签列表
        result = self.tag_mgr._normalize_tags(["tag1", "tag2"])
        assert result == {"tag1", "tag2"}

    def test_is_builtin_tag(self):
        """测试内置标签检查"""
        assert self.tag_mgr._is_builtin_tag(GLOBAL) is True
        assert self.tag_mgr._is_builtin_tag("tag1") is False