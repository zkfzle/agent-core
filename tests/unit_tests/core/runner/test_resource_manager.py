...
class TestResourceMgrAddMethodsValidation:
    ...
    def assert_status_code(self, err, expect_error, expect_messsage):
    ...
    async def test_add_agents_with_invalid_cards_and_providers(self, resource_mgr):
        ...
        self.assert_status_code(
            err,
            StatusCode.RESOURCE_PROVIDER_INVALID,
            "agent provider is invalid, reason='invalid card at idx 0: card cannot be None, must be an instance of AgentCard'")
        ...
        self.assert_status_code(
            err,
            StatusCode.RESOURCE_PROVIDER_INVALID,
            "agent provider is invalid, reason='invalid provider at idx 0: provider cannot be None, must be a callable function'")
        ...
        self.assert_status_code(
            err,
            StatusCode.RESOURCE_PROVIDER_INVALID,
            "agent provider is invalid, reason='invalid agent card type at idx 0: expected AgentCard, got str'")
    ...
    async def test_add_tool_with_invalid_tool(self, resource_mgr):
        ...
        self.assert_status_code(
            err,
            StatusCode.RESOURCE_VALUE_INVALID,
            "tool value is invalid, reason='tool list cannot be empty: expected a non-empty list of Tool'")
        ...
        self.assert_status_code(
            err,
            StatusCode.RESOURCE_VALUE_INVALID,
            "tool value is invalid, reason='invalid tool type at index 0: expected Tool, got Mock'")
