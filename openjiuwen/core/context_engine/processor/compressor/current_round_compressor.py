...
from openjiuwen.core.foundation.llm1 import (
    BaseMessage, AssistantMessage, SystemMessage, UserMessage, ToolMessage,
...
class CurrentRoundCompressor(ContextProcessor):
    ...
    async def on_add_messages(self,
        ...
        if self._single_multi_config:
            compressed_context = await self.multi_compress(context_messages,
            ...
            try:
                compressed_context = await self.single_compress(context_messages,
    ...
    async def compress(self, messages: List[BaseMessage]) -> Optional[BaseMessage]:
        ...
        if summary and isinstance(summary, dict):
            ...
            ai_message = AssistantMessage(content="[CUR-SUMMARY]" + summary)
            return ai_message
        else:
            return None
    ...
    async def run():
        ...
        for i in range(20):
            ...
            messages_batch.extend([
                UserMessage(content=f"{i}" * 100),
                AssistantMessage(tool_calls=[tool_call], content=f"{i}" * 1000),  # 关联ToolCall
                ToolMessage(tool_call_id=f"id_{i}", content=f"{i}" * 100),  # 和ToolCall.id一致
                AssistantMessage(content=f"{i}" * 100)
            ])

            # 每5次循环批量添加一次（减少压缩器触发次数，便于调试）
            if (i + 1) % 5 == 0:
