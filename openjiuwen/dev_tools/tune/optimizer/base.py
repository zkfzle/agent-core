...
class BaseOptimizer:
        ...
        for name, llm_call in parameters.items():
            if not llm_call:
                raise JiuWenBaseException(
                    StatusCode.PROMPT_OPTIMIZATION_OPTIMIZER_PARAMS_ERROR.code,
                    StatusCode.PROMPT_OPTIMIZATION_OPTIMIZER_PARAMS_ERROR.errmsg.format(
    ...
    def backward(self,
        ...
        try:
            ...
            traceback.print_exc()
            raise JiuWenBaseException(
                StatusCode.PROMPT_OPTIMIZATION_OPTIMIZER_BACKWORD_ERROR.code,
                StatusCode.PROMPT_OPTIMIZATION_OPTIMIZER_BACKWORD_ERROR.errmsg.format(
                    error_msg=f"{str(e)}"
                )
            )
    ...
    def update(self):
        ...
        try:
            ...
            self._history.clear_history()
            raise JiuWenBaseException(
                StatusCode.PROMPT_OPTIMIZATION_OPTIMIZER_UPDATE_ERROR.code,
                StatusCode.PROMPT_OPTIMIZATION_OPTIMIZER_UPDATE_ERROR.errmsg.format(
                    error_msg=f"{str(e)}"
                )
            )
    ...
    def _validate_parameters(self):
        if not self._parameters:
            raise JiuWenBaseException(
                StatusCode.PROMPT_OPTIMIZATION_AGENT_PARAMS_ERROR.code,
                StatusCode.PROMPT_OPTIMIZATION_AGENT_PARAMS_ERROR.errmsg.format(
