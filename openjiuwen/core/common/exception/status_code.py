...
class StatusCode(Enum):
    ...
    COMMON_SCHEMA_INVALID = (90000,
                             generate_error_message_template(scope="COMMON", subject="SCHEMA", failure_type="INVALID").template)
    COMMON_SCHEMA_CONFIG_ERROR = (90001,
                                  generate_error_message_template(scope="COMMON", subject="SCHEMA", failure_type="CONFIG_ERROR").template)

    # Workflow Component  100000 - 109999

    # Workflow: Interactive And Recovery 100000 - 100029
    WORKFLOW_INPUT_INVALID = (100000,
                              generate_error_message_template(scope="WORKFLOW", subject="INPUT", failure_type="INVALID").template)
    WORKFLOW_STATE_RUNTIME_ERROR = (100001,
                                    generate_error_message_template(scope="WORKFLOW", subject="STATE", failure_type="RUNTIME_ERROR").template)
    WORKFLOW_EXECUTION_NOT_SUPPORT = (100002,
                                      generate_error_message_template(scope="WORKFLOW", subject="EXECUTION", failure_type="NOT_SUPPORT").template)
    WORKFLOW_INTERRUPT_EXECUTION_ERROR = (100003,
                                          generate_error_message_template(scope="WORKFLOW", subject="INTERRUPT", failure_type="EXECUTION_ERROR").template)
    WORKFLOW_STREAM_NOT_SUPPORT = (100004,
                                   generate_error_message_template(scope="WORKFLOW", subject="STREAM", failure_type="NOT_SUPPORT").template)
    WORKFLOW_COMPONENT_RUNTIME_ERROR = (100005,
                                        generate_error_message_template(scope="WORKFLOW", subject="COMPONENT", failure_type="RUNTIME_ERROR").template)
    WORKFLOW_STATE_INVALID = (100006,
                              generate_error_message_template(scope="WORKFLOW", subject="STATE", failure_type="INVALID").template)

    # Workflow: Execution 100100 - 100199
    WORKFLOW_EXECUTION_RUNTIME_ERROR = (100100,
                                        generate_error_message_template(scope="WORKFLOW", subject="EXECUTION", failure_type="RUNTIME_ERROR").template)
    WORKFLOW_INVOKE_TIMEOUT = (100101,
                               generate_error_message_template(scope="WORKFLOW", subject="INVOKE", failure_type="TIMEOUT").template)
    WORKFLOW_STREAM_EXECUTION_TIMEOUT = (100102,
                                         generate_error_message_template(scope="WORKFLOW", subject="STREAM_EXECUTION", failure_type="TIMEOUT").template)
    ...
    COMPONENT_LLM_CONFIG_ERROR = (101002,
                                  generate_error_message_template(scope="COMPONENT", subject="LLM", failure_type="CONFIG_ERROR").template)
    ...
    MODEL_SERVICE_CONFIG_ERROR = (102001,
                                  generate_error_message_template(scope="MODEL", subject="SERVICE", failure_type="CONFIG_ERROR").template)
    MODEL_CONFIG_ERROR = (102002,
                          generate_error_message_template(scope="MODEL", subject="", failure_type="CONFIG_ERROR").template)
    MODEL_INVOKE_PARAM_ERROR = (102003,
                                generate_error_message_template(scope="MODEL", subject="INVOKE", failure_type="PARAM_ERROR").template)
    MODEL_CLIENT_CONFIG_INVALID = (102004,
                                   generate_error_message_template(scope="MODEL", subject="CLIENT_CONFIG", failure_type="INVALID").template)
