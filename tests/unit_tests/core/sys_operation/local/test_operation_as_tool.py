...
@pytest.mark.asyncio
async def test_fs_list_tools_with_dict(sys_op):
...
@pytest.mark.asyncio
async def test_fs_resource_mgr_read_write_text(sys_op, work_dir):
    ...
    card_id = "test_op"
    card = SysOperationCard(id=card_id)
...
@pytest.mark.asyncio
async def test_fs_resource_mgr_read_write_binary(sys_op, work_dir):
...
@pytest.mark.asyncio
async def test_fs_resource_mgr_other_methods(sys_op, work_dir):
...
@pytest.mark.asyncio
async def test_shell_resource_mgr_integration(sys_op):
    ...
    assert tool.card.name == f"{card_id}.shell.execute_cmd"

    # Verify tool execution through resource_mgr
    invoke_res = await tool.invoke({"command": "echo hello_integration", "options": {"encoding": "gbk"}})
