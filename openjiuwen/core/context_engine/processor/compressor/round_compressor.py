...
class RoundLevelCompressorConfig(BaseModel):
    ...
    preserve_latest_round: bool = Field(default=True)
    """
...
class RoundLevelCompressor(ContextProcessor):
    ...
    async def trigger_add_messages(
        ...
        filtered_rounds = filter_out_latest_round(rounds, self._preserve_latest_round)
    ...
    def _iter_rounds(self, messages: List[BaseMessage]):
        ...
        while i < len(messages) - 1:
            ...
            if (
                    ...
                    and (not isinstance(a, AssistantMessage) or not a.tool_calls)
            ):
                result.append(
                    DialogueRound(
                        user=u,
    ...
    def _find_best_round_window(self, rounds: List[DialogueRound]) -> List[List[DialogueRound]]:
        ...
        for r in rounds:
            ...
            if len(window) >= self._threshold:
                candidate = window[-self._threshold:]
