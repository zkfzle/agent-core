...
async def test_multi_stream_workflow():
    ...
    async for chunk in wf.stream({"inputs": [1, 2, 3]}, create_workflow_session(), stream_modes=[BaseStreamMode.OUTPUT]):
...
async def test_workflow_stream_with_exception():
    ...
    result = await workflow.invoke(inputs={"user_inputs": {"array": [1, 2, 3, 4, 5, 6, 7]}}, session=create_workflow_session())
...
async def test_node_with_dual_stream_abilities_transform_and_stream():
    ...
    async for chunk in flow.stream(user_inputs, session=create_workflow_session(), stream_modes=[BaseStreamMode.OUTPUT]):
...
async def test_dual_ability_node_with_stream_error():
    ...
    with pytest.raises(JiuWenBaseException) as exc_info:
        async for _ in flow.stream(user_inputs, session=create_workflow_session(), stream_modes=[BaseStreamMode.OUTPUT]):
            pass
