    ...
    def _prepare_detection_inputs(self, inputs, chat_history):
        ...
        if self._default_config.enable_input:
            try:
                ...
                raise build_error(StatusCode.COMPONENT_INTENT_DETECTION_INPUT_PARAM_ERROR, error_msg=ExceptionUtils.format_validation_error(e))
    ...
    def _invoke_llm_and_get_result(self, current_inputs):
        ...
        try:
            ...
            raise build_error(StatusCode.COMPONENT_INTENT_DETECTION_INVOKE_CALL_FAILED,
                                           error_msg="failed to invoke llm and get result", cause=e)
        if UserConfig.is_sensitive():
