...
class VariableManager(BaseMemoryManager):
    ...
    async def update_user_variable(self, user_id: str, scope_id: str, var_name: str, var_mem: str):
        if self.kv_store is None:
            memory_logger.error(
                "kv_store cannot be None",
                event_type=LogEventType.MEMORY_STORE,
                operation="update user variable",
                memory_type="variable",
                metadata={"user_id": user_id, "scope_id": scope_id}
            )
            return
    ...
    async def delete_user_variable(self, user_id: str, scope_id: str, var_name: str):
        if self.kv_store is None:
            memory_logger.error(
                "kv_store cannot be None",
                event_type=LogEventType.MEMORY_STORE,
                operation="delete user variable",
                memory_type="variable",
                metadata={"user_id": user_id, "scope_id": scope_id}
            )
            return
    ...
    def _check_user_and_scope_id(user_id, scope_id, context="Operation"):
        if not user_id or not user_id.strip():
            memory_logger.error(
                "Operation failed, user ID is empty",
                event_type=LogEventType.MEMORY_VALIDATE,
                operation="check user and scope id",
                memory_type="variable",
                metadata={"user_id": user_id, "scope_id": scope_id, "context": context},
            )
        if not scope_id or not scope_id.strip():
            memory_logger.error(
                "Operation failed, scope ID is empty",
                event_type=LogEventType.MEMORY_VALIDATE,
                operation="check user and scope id",
                memory_type="variable",
                metadata={"user_id": user_id, "scope_id": scope_id, "context": context},
