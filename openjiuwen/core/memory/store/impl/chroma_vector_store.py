...
class ChromaVectorStore(VectorStore):
    ...
    def _check_table_name(self, table_name: Optional[str] = None, operation: Optional[str] = None):
        """check table name"""
        if table_name is None or table_name.strip() == "":
            raise ValueError(f"Chroma collection name is required for {operation}")

    def is_collection_exists(self, table_name: str) -> bool:
        """Check whether the collection exists"""
        try:
            self.client.get_collection(table_name)
            return True
    ...
    async def add(
        ...
        **kwargs: Any,
        ...
        self._check_table_name(table_name, "add")
    ...
    async def search(
        ...
        **kwargs: Any,
        ...
        self._check_table_name(table_name, "search")
        ...
        if results['ids'] and len(results['ids']) > 0 and results['ids'][0]:
            ...
            for idx, (id, distance, doc, metadata) in enumerate(zip(ids, distances, documents, metadatas)):
                score = 1 - distance
    ...
    async def delete(
        ...
        **kwargs: Any,
        ...
        """Delete vectors"""
        table_name = kwargs.get("table_name")
        self._check_table_name(table_name, "delete")
