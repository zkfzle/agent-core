...
class CallableSchemaExtractor:
    ...
    @classmethod
    def generate_schema(cls, callable: Callable) -> Dict:
        """Generate complete callable schema"""
        signature = inspect.signature(callable)
        type_hints = get_type_hints(callable)
        return cls._generate_input_schema(callable, signature, type_hints)


    @classmethod
    def _get_type_schema(cls, type_hint: Any) -> Dict:
    ...
    @staticmethod
    def _get_base_model_schema(model: Type[BaseModel]) -> Dict:
    ...
    @staticmethod
    def _get_enum_schema(enum_type: Type[Enum]) -> Dict:
        """Handle enum types"""
        return {
            "type": "string",
    ...
    @classmethod
    def _generate_input_schema(cls, callable: Callable, signature, type_hints) -> Dict:
        """Generate input JSON Schema with all definitions expanded inline"""
        properties = {}
        required = []

        param_descriptions = cls._extract_docstring_descriptions(callable)
        ...
        for param_name, param in signature.parameters.items():
            ...
            param_schema = cls._get_type_schema(param_type)
        ...
        schema = {
            ...
            "title": cls._humanize_name(callable.__name__)
        }
