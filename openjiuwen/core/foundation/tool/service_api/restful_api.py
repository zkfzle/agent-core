...
class RestfulApiCard(ToolCard):
    ...
    response_batch_bytes_size: int = Field(default=10 * 1024 * 1024, description="Response batch size in bytes")
...
class RestfulApi(Tool):
    ...
    def __init__(self, card: RestfulApiCard):
        ...
        self._response_bytes_size_limit = card.response_batch_bytes_size
    ...
    async def _async_request(self, map_results: dict, timeout: float, max_response_size: int):
        request_arg = {}
        ...
        query_params = map_results.get(APIParamLocation.QUERY)
        if query_params:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.request(
                ...
                response_data = await self._format_response(response, max_response_size)
    ...
    async def invoke(self, inputs: Input, **kwargs) -> Output:
        try:
            ...
            return await self._async_request(map_results,
                                             ...
                                             kwargs.get("response_batch_bytes_size", self._response_bytes_size_limit))
