...
from openjiuwen.core.foundation.llm1.model import _CLIENT_TYPE_REGISTRY
...
async def test_feedback_prompt_builder_select():
    ...
    prompt = "你是一个旅行助手"
    feedback = "丰富一下"
    response = await builder.build(prompt=prompt + MOCK_INTENT, feedback=feedback, mode="select", start_pos=0, end_pos=3)
    # 构建预期的消息内容
