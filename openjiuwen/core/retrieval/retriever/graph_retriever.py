    ...
    def _ensure_mode_allowed(self, mode: Literal["vector", "sparse", "hybrid"]) -> None:
        ...
        if allowed is None:
            raise JiuWenBaseException(
                ...
                StatusCode.RETRIEVAL_RETRIEVER_INDEX_TYPE_NOT_SUPPORT.errmsg.format(error_msg=f"Unsupported index_type={self.index_type}"),
            )
        if mode not in allowed:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_RETRIEVER_MODE_INVALID.code,
                StatusCode.RETRIEVAL_RETRIEVER_MODE_INVALID.errmsg.format(
                    error_msg=f"mode={mode} is incompatible with index_type={self.index_type}; allowed modes: {sorted(allowed)}"
                ),
            )
    ...
    def _get_retriever_for_mode(
        ...
        if not self.vector_store:
            raise JiuWenBaseException(
                ...
                StatusCode.RETRIEVAL_RETRIEVER_VECTOR_STORE_NOT_FOUND.errmsg.format(error_msg="vector_store is required for dynamic retriever creation"),
            )
        ...
        if mode == "vector":
            ...
            if not self.embed_model:
                raise JiuWenBaseException(
                    ...
                    StatusCode.RETRIEVAL_RETRIEVER_EMBED_MODEL_NOT_FOUND.errmsg.format(error_msg="embed_model is required for vector mode"),
                )
