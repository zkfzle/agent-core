...
class TestProgressReporter(unittest.TestCase):
    ...
    def test_calculate_progress(self):
        """Test progress calculation for different agent types."""
        llm_reporter = ProgressReporter(self.session_id, "llm_agent")
        wf_reporter = ProgressReporter(self.session_id, "workflow")

        # LLM agent stages
        self.assertEqual(llm_reporter._calculate_progress(ProgressStage.INITIALIZING), 0.0)
        self.assertEqual(llm_reporter._calculate_progress(ProgressStage.RESOURCE_RETRIEVING), 20.0)

        # Workflow stages
        self.assertEqual(wf_reporter._calculate_progress(ProgressStage.INITIALIZING), 0.0)
        self.assertEqual(wf_reporter._calculate_progress(ProgressStage.VALIDATING), 70.0)
