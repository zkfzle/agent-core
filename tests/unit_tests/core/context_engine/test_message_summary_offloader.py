...
from openjiuwen.core.context_engine.schema.messages import OffloadMessage
...
class TestMessageSummaryOffloader:
    ...
    async def test_init_with_default_config(self, default_config):
        ...
        with patch('openjiuwen.core.context_engine.processor.offloader.message_summary_offloader.Model') as mock_model_class:
            ...
            assert offloader._model == mock_model_instance
    ...
    async def test_offload_message_with_default_prompt(self, default_config):
        ...
        with patch('openjiuwen.core.context_engine.processor.offloader.message_summary_offloader.Model') as mock_model_class:
            ...
            offloader = MessageSummaryOffloader(default_config)
            context = SessionModelContext("context_id", "session_id", ContextEngineConfig(),
                                            history_messages=[])
            result = await offloader._offload_message(original_message, context)
            
            # Verify result is a OffloadMessage
            ...
            assert isinstance(result, OffloadMessage)
