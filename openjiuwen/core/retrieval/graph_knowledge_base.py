...
class GraphKnowledgeBase(KnowledgeBase):
    ...
    async def add_documents(
        ...
        **kwargs: Any,
        ...
        if not success:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_KB_CHUNK_INDEX_BUILD_EXECUTION_ERROR.code, "Failed to build chunk index"
            )
        ...
        if self.config.use_graph and self.extractor:
            ...
            if triples:
                ...
                if not success:
                    raise JiuWenBaseException(
                        StatusCode.RETRIEVAL_KB_TRIPLE_INDEX_BUILD_EXECUTION_ERROR.code, "Failed to build triple index"
                    )
                else:
        ...
        if retrieval_config.use_graph or self.config.use_graph:
            if not self.graph_retriever:
                if not self.vector_store:
                    raise JiuWenBaseException(
                        StatusCode.RETRIEVAL_KB_VECTOR_STORE_NOT_FOUND.code, "vector_store is required for graph retrieval"
                    )
