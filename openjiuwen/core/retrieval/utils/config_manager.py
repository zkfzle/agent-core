    ...
    def load_from_file(self, path: str) -> None:
        ...
        if not path_obj.exists():
            raise JiuWenBaseException(
                ...
                StatusCode.RETRIEVAL_UTILS_CONFIG_FILE_NOT_FOUND.errmsg.format(error_msg=f"Configuration file does not exist: {path}"),
            )
    ...
    def get_knowledge_base_config(self) -> KnowledgeBaseConfig:
        ...
        if not config:
            raise JiuWenBaseException(
                ...
                StatusCode.RETRIEVAL_UTILS_CONFIG_PROCESS_ERROR.errmsg.format(error_msg="Knowledge base configuration not loaded"),
            )
        return config
