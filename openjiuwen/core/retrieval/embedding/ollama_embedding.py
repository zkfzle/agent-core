...
class OllamaEmbedding(Embedding):
    ...
    async def embed_query(self, text: str, **kwargs: Any) -> List[float]:
        if not text.strip():
            raise JiuWenBaseException(
                ...
                StatusCode.RETRIEVAL_EMBEDDING_INPUT_INVALID.errmsg.format(error_msg="Empty text provided for embedding"),
            )
    ...
    async def embed_documents(
        ...
        **kwargs: Any,
        ...
        if not isinstance(callback_cls, type) or not issubclass(callback_cls, BaseCallback):
            raise JiuWenBaseException(
                ...
                StatusCode.RETRIEVAL_EMBEDDING_CALLBACK_INVALID.errmsg.format(
                    error_msg=f"callback_cls in OllamaEmbedding.embed_documents must be a subclass of BaseCallback, got {type(callback_cls)}"
                ),
            )
        ...
        if not non_empty_texts:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_EMBEDDING_INPUT_INVALID.code,
                StatusCode.RETRIEVAL_EMBEDDING_INPUT_INVALID.errmsg.format(error_msg="All texts are empty after filtering"),
            )
    ...
    async def _get_ollama_embedding(self, text: str | List[str], **kwargs) -> List[List[float]]:
        ...
        if not text:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_EMBEDDING_INPUT_INVALID.code,
                StatusCode.RETRIEVAL_EMBEDDING_INPUT_INVALID.errmsg.format(error_msg="Empty text or list provided for embedding"),
            )
        ...
        raise JiuWenBaseException(
            ...
            StatusCode.RETRIEVAL_EMBEDDING_UNREACHABLE_CALL_FAILED.errmsg.format(error_msg="This should never be reached"),
        )
