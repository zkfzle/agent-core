...
class TestChromaVectorStore:
    ...
    def test_chroma_result_to_search_results_vector_mode(self, temp_chroma_path, vector_store_config):
        """测试转换 ChromaDB 结果为 SearchResult（向量模式）"""
        store = ChromaVectorStore(config=vector_store_config, chroma_path=temp_chroma_path)
        ...
        search_results = store._chroma_result_to_search_results(results, mode="vector")
        assert len(search_results) == 2
        assert search_results[0].text == "Content 1"
        assert search_results[0].score > 0  # 应该转换为相似度分数

    def test_chroma_result_to_search_results_sparse_mode(self, temp_chroma_path, vector_store_config):
        """测试转换 ChromaDB 结果为 SearchResult（稀疏模式）"""
        store = ChromaVectorStore(config=vector_store_config, chroma_path=temp_chroma_path)
        ...
        search_results = store._chroma_result_to_search_results(results, mode="sparse")
        assert len(search_results) == 1
        assert search_results[0].text == "Content 1"

    def test_chroma_result_to_search_results_empty(self, temp_chroma_path, vector_store_config):
        """测试转换空结果"""
        store = ChromaVectorStore(config=vector_store_config, chroma_path=temp_chroma_path)
    ...
    def test_chroma_result_to_search_results_with_sparse_vector(self, temp_chroma_path, vector_store_config):
        """测试转换包含稀疏向量的结果"""
        store = ChromaVectorStore(config=vector_store_config, chroma_path=temp_chroma_path)
        
        sparse_vec_json = json.dumps({"term1": 0.5, "term2": 0.3})
        results = {
            "ids": [["1"]],
            "documents": [["Content 1"]],
            "metadatas": [[{"sparse_vector": sparse_vec_json}]],
            "distances": [[0.1]],
        }
        ...
        assert len(search_results) == 1
        # 稀疏向量应该被解析
        if "sparse_vector" in search_results[0].metadata:
            sparse_vec = search_results[0].metadata["sparse_vector"]
            assert isinstance(sparse_vec, (dict, list))
