...
async def test_simple_workflow():
    ...
    assert e.value.message == StatusCode.COMPONENT_EXECUTION_RUNTIME_ERROR.errmsg.format(node_id="a", ability="invoke", error=RuntimeError('value < 20'))
...
async def test_workflow_comp():
    ...
    with pytest.raises(JiuWenBaseException) as e:
        await flow.invoke({"inputs": {"a": 1, "b": "haha"}}, WorkflowSession(session_id=session_id))
    assert e.value.error_code == StatusCode.COMPONENT_EXECUTION_RUNTIME_ERROR.code
    assert e.value.message == StatusCode.COMPONENT_EXECUTION_RUNTIME_ERROR.errmsg.format(node_id="a2", ability="invoke", error=RuntimeError('value < 20'))
...
async def test_simple_interactive_workflow_both_raw_input_update():
        ...
        assert exc_info.value.error_code == StatusCode.WORKFLOW_STATE_EXECUTION_ERROR.code
        assert exc_info.value.message == StatusCode.WORKFLOW_STATE_EXECUTION_ERROR.errmsg
