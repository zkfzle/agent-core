...
class LongTermMemory(metaclass=Singleton):
    ...
    async def add_messages(
        ...
        if not self._validate_id(user_id, scope_id):
            logger.error(f"Invalid scope_id or user_id format, scope_id={scope_id}, user_id={user_id}")
            return
        msg_id = "-1"
    ...
    @staticmethod
    def _validate_id(user_id: str = "", scope_id: str = "") -> bool:
        """
        ...
        if not scope_id or not user_id:
            logger.error(f"scope_id or user_id is invalid: {scope_id}, {user_id}")
            return False
        if "/" in scope_id or "/" in scope_id:
            logger.error(f"scope_id or user_id cannot contain separator '/', scope_id={scope_id}, user_id={user_id}")
            return False
        if len(scope_id) > 128 or len(user_id) > 128:
            logger.error(f"scope_id or user_id length exceeds limit (128), scope_id={scope_id}, user_id={user_id}")
            return False
        return True