...
from openjiuwen.core.foundation.llm1 import BaseModelInfo, AssistantMessage, UsageMetadata, ModelConfig, ToolCall
...
class TestReActAgentWithWorkflowInterruptMock(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_react_agent_invoke_with_workflow_interrupt_mock(self):
        ...
        with patch(
                'openjiuwen.core.foundation.llm1.model.Model.invoke',
                side_effect=mock_llm.invoke
        ), patch(
                'openjiuwen.core.foundation.llm1.model.Model.stream',
            ...
            print(f"[PASS] di yi ci diao yong jiao yan tong guo: fan hui jiao hu qing qiu, ti shi: {result[0].payload.value}")
