...
- TaskManager: Task manager responsible for CRUD operations, status management, priority management, and hierarchical relationship management
...
class TaskManager:
    ...
    Responsible for task CRUD operations, status management, priority management, and hierarchical relationship management.
    ...
    def remove_task(
        ...
        Args:
            task_id: Task ID, can be a single ID or a list of IDs. If None, must provide other filter criteria (session_id, priority, status, is_root, etc.), otherwise ValueError will be raised to prevent accidental deletion of all tasks
            session_id: Session ID
            user_id: User ID
            priority: Priority
            status: Task execution status
