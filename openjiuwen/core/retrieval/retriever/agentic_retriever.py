...
class AgenticRetriever(Retriever):
    ...
    def __init__(
        ...
        if graph_retriever is None:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_RETRIEVER_GRAPH_RETRIEVER_NOT_FOUND.code, "graph_retriever is required for AgenticRetriever"
            )
        if llm_client is None:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_RETRIEVER_LLM_CLIENT_NOT_FOUND.code, "llm_client is required for AgenticRetriever"
            )
    ...
    async def retrieve(
        ...
        **kwargs: Any,
        ...
        if top_k is None:
            raise JiuWenBaseException(
                StatusCode.RETRIEVAL_RETRIEVER_TOP_K_NOT_FOUND.code, "top_k is required for AgenticRetriever"
            )
        topk = top_k
