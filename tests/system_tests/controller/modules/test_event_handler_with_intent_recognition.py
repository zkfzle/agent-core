...
class TestEventHandlerWithIntentRecognition(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_process_create_task_intent(self):
        """Test processing CREATE_TASK intent directly"""
        await self.handler._process_create_task_intent(self.sample_create_intent, self.mock_session)
        
        # Verify task was created with correct attributes
        self.mock_task_manager.add_task.assert_called_once()
        call_args = self.mock_task_manager.add_task.call_args[0][0]
        self.assertEqual(call_args.task_id, "task1")
        self.assertEqual(call_args.session_id, "test_session_id")
        self.assertEqual(call_args.description, "Test task description")
        self.assertEqual(call_args.status, TaskStatus.SUBMITTED)
        self.assertEqual(call_args.context_id, "test_session_id_task1")

    async def test_process_pause_task_intent(self):
        """Test processing PAUSE_TASK intent directly"""
        await self.handler._process_pause_task_intent(self.sample_pause_intent, self.mock_session)
        
        self.mock_task_scheduler.pause_task.assert_called_once_with("task1")

    async def test_process_resume_task_intent_paused(self):
        """Test processing RESUME_TASK intent for paused task"""
        paused_task = Task(
            session_id="test_session_id",
            task_id="task1",
            task_type="test_task",
        ...
        await self.handler._process_resume_task_intent(self.sample_resume_intent, self.mock_session)
        
        self.mock_task_manager.update_task.assert_called_once()
        updated_task = self.mock_task_manager.update_task.call_args[0][0]
        self.assertEqual(updated_task.status, TaskStatus.SUBMITTED)

    async def test_process_continue_task_intent(self):
        """Test processing CONTINUE_TASK intent"""
        dependent_task = Task(
            session_id="test_session_id",
            task_id="task1",
            task_type="test_task",
        ...
        await self.handler._process_continue_task_intent(self.sample_continue_intent, self.mock_session)
    ...
    async def test_process_continue_task_intent_invalid_event(self):
        """Test processing CONTINUE_TASK intent with invalid event type"""
        invalid_intent = Intent(
            intent_type=IntentType.CONTINUE_TASK,
            event=TaskCompletionEvent(
                task_result=[
                    TextDataFrame(text="Task Completed")
                ]
            ),
            target_task_id="task2",
            depend_task_id=["task1"]
        ...
        with self.assertRaises(Exception):
            await self.handler._process_continue_task_intent(invalid_intent, self.mock_session)

    async def test_process_supplement_task_intent(self):
        """Test processing SUPPLEMENT_TASK intent"""
        existing_task = Task(
            session_id="test_session_id",
            task_id="task1",
            task_type="test_task",
        ...
        await self.handler._process_supplement_task_intent(self.sample_supplement_intent, self.mock_session)
    ...
    async def test_process_supplement_task_intent_wrong_type(self):
        """Test processing SUPPLEMENT_TASK intent with wrong intent type"""
        wrong_intent = Intent(
            intent_type=IntentType.CREATE_TASK,
            event=self.sample_input_event,
            target_task_id="task1",
            target_task_description="Original task",
        )
        
        with self.assertRaises(Exception):
            await self.handler._process_supplement_task_intent(wrong_intent, self.mock_session)

    async def test_process_cancel_task_intent(self):
        """Test processing CANCEL_TASK intent"""
        await self.handler._process_cancel_task_intent(self.sample_cancel_intent, self.mock_session)
        
        self.mock_task_scheduler.cancel_task.assert_called_once_with("task1")

    async def test_process_cancel_task_intent_wrong_type(self):
        """Test processing CANCEL_TASK intent with wrong intent type"""
        wrong_intent = Intent(
            intent_type=IntentType.CREATE_TASK,
            event=self.sample_input_event,
            target_task_id="task1",
        ...
        with self.assertRaises(Exception):
            await self.handler._process_cancel_task_intent(wrong_intent, self.mock_session)

    async def test_process_modify_task_intent(self):
        """Test processing MODIFY_TASK intent"""
        existing_task = Task(
            session_id="test_session_id",
            task_id="task1",
            task_type="test_task",
        ...
        await self.handler._process_modify_task_intent(self.sample_modify_intent, self.mock_session)
    ...
    async def test_process_modify_task_intent_with_existing_inputs(self):
        """Test processing MODIFY_TASK intent when task has existing inputs"""
        existing_task = Task(
            session_id="test_session_id",
            task_id="task1",
            task_type="test_task",
        ...
        updated_task = self.mock_task_manager.update_task.call_args[0][0]
        self.assertIn(self.sample_input_event, updated_task.inputs)

    async def test_process_modify_task_intent_wrong_type(self):
        """Test processing MODIFY_TASK intent with wrong intent type"""
        wrong_intent = Intent(
            intent_type=IntentType.CREATE_TASK,
            event=self.sample_input_event,
            target_task_id="task1",
        ...
        with self.assertRaises(Exception):
            await self.handler._process_modify_task_intent(wrong_intent, self.mock_session)

    async def test_process_unknown_task_intent(self):
        """Test processing UNKNOWN_TASK intent"""
        await self.handler._process_unknown_task_intent(self.sample_unknown_intent, self.mock_session)
        
        self.mock_session.write_stream.assert_called_once()
        call_args = self.mock_session.write_stream.call_args[0][0]
        self.assertEqual(call_args["clarification_prompt"], "Could you please clarify?")

    async def test_process_unknown_task_intent_wrong_type(self):
        """Test processing UNKNOWN_TASK intent with wrong intent type"""
        wrong_intent = Intent(
            intent_type=IntentType.CREATE_TASK,
            event=self.sample_input_event,
            target_task_id="task1",
        ...
        with self.assertRaises(ValueError):
            await self.handler._process_unknown_task_intent(wrong_intent, self.mock_session)
