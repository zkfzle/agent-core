...
class CollectiveEvent(PriorityEvent):
    priority: int = Field(default=1, description="Priority of task")

    @classmethod
    async def async_create_tasks(cls, goal: str, llm, assignee=None, task_type="general", max_num=10, existing_tasks=None):
        existing_tasks_str = "\n".join([f"- {task.task_description}" for task in existing_tasks]) if existing_tasks else "None"
        ...
        prompt = prompt_template.get("generate_collect_task").format(query=goal, existing_tasks=existing_tasks_str, max_num=max_num)
