...
class TestChromaVectorStore:
    """ChromaVectorStore测试类"""

    def test_init_success(self, persist_directory):
        """测试初始化成功"""
    ...
    def test_get_collection(self, chroma_store, persist_directory, table_name):
        """测试获取/创建集合"""
        # 第一次调用_get_collection应该创建集合
        collection = chroma_store._get_collection(table_name)
        ...
        # 第二次调用_get_collection应该从缓存中获取
        collection2 = chroma_store._get_collection(table_name)
        assert collection2 is collection

    def test_remove_collection_from_cache(self, chroma_store, persist_directory, table_name):
        """测试从缓存中移除集合"""
        # 添加集合到缓存
        chroma_store._get_collection(table_name)
        assert table_name in chroma_store.collection_cache

        # 从缓存中移除集合
        chroma_store._remove_collection_from_cache(table_name)
        assert table_name not in chroma_store.collection_cache

    def test_check_table_name(self, chroma_store):
        """测试检查表名"""
        # 测试空表名应该抛出异常
        with pytest.raises(ValueError, match="Chroma collection name is required for test_operation"):
            chroma_store._check_table_name(None, "test_operation")

        with pytest.raises(ValueError, match="Chroma collection name is required for test_operation"):
            chroma_store._check_table_name("", "test_operation")

        with pytest.raises(ValueError, match="Chroma collection name is required for test_operation"):
            chroma_store._check_table_name("   ", "test_operation")

    def test_is_collection_exists(self, chroma_store, persist_directory, table_name):
        """测试集合是否存在"""
        # 测试集合不存在
        assert chroma_store.is_collection_exists(table_name) is False

        # 创建集合
        chroma_store._get_collection(table_name)

        # 测试集合存在
        assert chroma_store.is_collection_exists(table_name) is True
    ...
    async def test_delete_table(self, chroma_store, persist_directory, table_name):
        ...
        # 验证删除成功
        assert result is True
        assert not chroma_store.is_collection_exists(table_name)
        assert table_name not in chroma_store.collection_cache
