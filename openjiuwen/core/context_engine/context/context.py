    ...
    def pop_messages(self, size: int = 1, with_history: bool = True) -> List[BaseMessage]:
        if size is not None and size < 0:
            raise JiuWenBaseException(
                StatusCode.CONTEXT_OPERATION_ERROR.code,
                StatusCode.CONTEXT_OPERATION_ERROR.errmsg.format(
        ...
        if isinstance(messages, list):
            for msg in messages:
                if not isinstance(msg, BaseMessage):
                    raise JiuWenBaseException(
                        StatusCode.CONTEXT_VALIDATION_ERROR.code,
                        StatusCode.CONTEXT_VALIDATION_ERROR.errmsg.format(
