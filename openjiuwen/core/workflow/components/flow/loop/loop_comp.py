...
class AdvancedLoopComponent(ComponentComposable, LoopController, Executable, AtomicNode):
    ...
    def __init__(self, body: Executable,
                 condition: Union[str, Callable[[], bool], Condition] = None, break_nodes: list[LoopBreakComponent] = None,
    ...
    async def on_invoke(self, inputs: Input, session: BaseSession, **kwargs) -> Output:
        ...
        state = loop_session.state().get_io_state().get_state()
        ...
        loop_session.state().get_io_state().update_by_id(self._node_id, {self._node_id: None})
        return result
