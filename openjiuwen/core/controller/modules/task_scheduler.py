...
class TaskExecutor(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            ability_manager: Ability manager that contains available tools, workflows, etc.
            context_engine: Context engine used to manage conversation context
            task_manager: Task manager used to update task status
...
class TaskExecutorRegistry:
    ...
    def add_task_executor(
        ...
        Args:
            ...
            task_executor_builder: Task executor builder that receives config and dependencies and returns a TaskExecutor instance
        """
...
class TaskScheduler:
    ...
    async def execute_task(self, task_id: str, session: Session):
        ...
        if not tasks:
            ...
            raise build_error(
                StatusCode.CONTROLLER_TASK_EXECUTION_ERROR,
    ...
    def _are_all_tasks_completed(self, session_id: str) -> bool:
    ...
    async def _ensure_session_completion_signal(self, session_id: str):
        ...
        try:
            ...
            if not self._are_all_tasks_completed(session_id):
    ...
    async def _publish_task_event(
        ...
        Automatically determines event type based on chunk.payload.type, extracts data and constructs the corresponding event object.
    ...
    async def pause_task(self, task_id: str, session: Optional[Session] = None) -> bool:
        ...
        Execution flow:
            ...
            2. Get the executor of the current task and call can_pause; if it cannot be paused, return the reason; otherwise continue
        ...
        async with self._lock:
            ...
            # 4. Check whether it can be paused
            if executor:
                try:
            ...
            # 8. Clean up running task records
            del self._running_tasks[task_id]
    ...
    async def cancel_task(self, task_id: str, session: Optional[Session] = None) -> bool:
        ...
        Execution flow:
        1. Check whether the current task is running; if not, return directly
        2. Get the executor of the current task and call can_cancel; if it cannot be canceled, return the reason; otherwise continue
        ...
        async with self._lock:
            ...
            # 4. Check whether it can be canceled
            if executor:
                try:
