...
class ConfigManger:
    def __init__(self, default_config: Config):
        ...
        self._app_config: dict[str, Config] = {}  # Application-level parameters(medium priority)
        self._applock = Lock()
    ...
    def set_app_config(self, app_id: str, config_key: str, config_value: Any) -> None:
        """Set application-level parameters (will override default parameters)"""
        with self._applock:
            if app_id not in self._app_config:
                self._app_config[app_id] = self._default_config.model_copy()
            self._app_config[app_id] = self.update_nested_model(self._app_config[app_id], {config_key : config_value})
