...
"""
测试 LLM Agent 与 Workflow 中断功能

本测试用例验证 LLM Agent 在 workflow 中断场景下的行为。
"""
import os
import unittest
...
class TestReActAgentInterrupt(unittest.IsolatedAsyncioTestCase):
    """测试 ReAct Agent 中断功能"""
    ...
    def _create_prompt_template():
        ...
        return [
            dict(
                role="system",
                content=system_prompt.format(build_current_date())
            )
        ]

    @pytest.mark.asyncio
    @patch(
        "openjiuwen.single_agent.llm_agent.llm_controller."
        "LLMController._generate_plan_from_llm"
    )
    @patch(
        "openjiuwen.core.workflow.component.basic_components."
        "questioner_comp.QuestionerDirectReplyHandler._invoke_llm_for_extraction"
    )
    @patch(
        "openjiuwen.core.workflow.component.basic_components."
        "questioner_comp.QuestionerDirectReplyHandler._build_llm_inputs"
    )
    @patch("openjiuwen.core.foundation.llm.model.Model")
    async def test_react_agent_invoke_with_workflow_interrupt(
        self,
        mock_get_model,
        mock_llm_inputs,
        mock_extraction,
        mock_generate_plan_from_llm
    ):
        """测试 ReAct Agent 调用带中断的 workflow"""
        ...
        questioner_component = QuestionerComponent(
            questioner_comp_config=questioner_config
        )
    ...
    async def test_real_react_agent_invoke_with_workflow_interrupt(
        self,
        mock_set_scope,
        mock_get_model
    ):
        """测试真实 ReAct Agent 调用带中断的 workflow"""
        mock_get_model.return_value = MockLLMModel()
    ...
    @patch("openjiuwen.core.foundation.llm.model.Model")
    async def test_real_workflow_agent_invoke_with_workflow_interrupt(
        self,
        mock_get_model,
        mock_extraction
    ):
        """测试 WorkflowAgent 调用带中断的 workflow"""
        mock_get_model.return_value = MockLLMModel()
        ...
        if (isinstance(result, List) and
            isinstance(result[0], OutputSchema) and
    ...
    @patch("openjiuwen.core.foundation.llm.model.Model")
    async def test_real_workflow_agent_stream_with_workflow_interrupt(
        self,
        mock_get_model,
        mock_extraction
    ):
        """测试 WorkflowAgent 流式调用带中断的 workflow"""
        mock_get_model.return_value = MockLLMModel()
        mock_extraction.return_value = {"location": "hangzhou", "time": "today"}
        ...
        async for result in workflow_agent.stream({
            "conversation_id": "12345",
            "query": "查询今天天气"
        }):
            print(f"WorkflowAgent stream 第一次输出结果：{result}")
            if (isinstance(result, OutputSchema) and
                result.type == "__interaction__"):
