...
def build_data_analysis_task_executor(
    ...
    Args:
        config: controller configuration including scheduling and timeout
            parameters
        ability_manager: ability manager providing tools and capabilities
            required for data analysis
        context_engine: context engine used for data storage and retrieval
        task_manager: task manager used to manage task lifecycle
        event_queue: event queue used to publish and subscribe to events
...
class DeepSearchAgentTest(unittest.IsolatedAsyncioTestCase):
    async def test_deepsearch_end_to_end(self):
        ...
        print("✅ test_deepsearch_end_to_end passed")
    ...
    async def test_deepsearch_end_to_end_invoke(self):
        ...
        agent = await build_deepsearch_agent(agent_card)
        session = TaskSession(trace_id="example_deepsearch")
        ...
        assert full_output.count("成功调用handle_task_completion回调") == 3, \
            "断言失败：handle_task_completion 回调未被调用 3 次"
        print("✅ test_deepsearch_end_to_end_invoke passed")
