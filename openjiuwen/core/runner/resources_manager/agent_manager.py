...
class AgentMgr(AbstractManager[AgentWithSession]):
    ...
    def add_agent(self, agent_id: str, agent: Union[BaseAgent, AgentProvider, RemoteAgent, AgentAdapter]) -> None:
        ...
        def validate_agent(agent_obj):
            ...
            return AgentWithSession(
                session=StaticAgentSession(config=agent_obj.config(), resource_mgr=self._resource_manager),
                agent=agent_obj
            )
    ...
    def remove_agent(self, agent_id: str) -> Optional[BaseAgent | RemoteAgent | AgentAdapter]:
        ...
        agent_with_session = self._remove_resource(agent_id, StatusCode.SESSION_AGENT_REMOVE_FAILED)
        if isinstance(agent_with_session, (RemoteAgent, AgentAdapter)):
            return agent_with_session
        return agent_with_session.agent if agent_with_session else None
    ...
    def get_agent(self, agent_id: str) -> Optional[AgentWithSession | RemoteAgent]:
        ...
        def create_agent_from_provider(provider):
            ...
            return AgentWithSession(
                session=StaticAgentSession(config=agent.config(), resource_mgr=self._resource_manager),
                agent=agent
            )
