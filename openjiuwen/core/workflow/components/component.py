...
class ComponentExecutable(Executable):
    ...
    async def invoke(self, inputs: Input, session: Session, context: ModelContext) -> Output:
        ...
        raise NotImplementedError(
            f"The '{method_name}' method must be implemented by '{class_name}' subclass. "
            f"This method is required for component execution."
        )
...
class ComponentComposable(ABC):
    ...
    def to_executable(self) -> Executable:
        ...
        if isinstance(self, Executable):
            return self
        raise build_error(StatusCode.METHOD_NOT_IMPLEMENTED, method="to_executable", class_name=type(self).__name__)
