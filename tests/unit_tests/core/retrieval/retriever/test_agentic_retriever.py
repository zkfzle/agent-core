...
class TestAgenticRetriever:
    ...
    def test_init_default_mode_vector(self):
        """测试默认模式（vector index_type）"""
        mock_graph = MagicMock()
        mock_graph.index_type = "vector"
        
        retriever = AgenticRetriever(
            graph_retriever=mock_graph,
            llm_client=MagicMock(),
        )
        assert retriever._default_mode == "vector"

    def test_init_default_mode_bm25(self):
        """测试默认模式（bm25 index_type）"""
        mock_graph = MagicMock()
        mock_graph.index_type = "bm25"
        
        retriever = AgenticRetriever(
            graph_retriever=mock_graph,
            llm_client=MagicMock(),
        )
        assert retriever._default_mode == "sparse"

    def test_init_default_mode_hybrid(self):
        """测试默认模式（hybrid index_type）"""
        mock_graph = MagicMock()
        mock_graph.index_type = "hybrid"
        
        retriever = AgenticRetriever(
            graph_retriever=mock_graph,
            llm_client=MagicMock(),
        )
        assert retriever._default_mode == "hybrid"

    def test_init_default_mode_no_index_type(self):
        """测试默认模式（无 index_type）"""
        mock_graph = MagicMock()
        mock_graph.index_type = None
    ...
    @pytest.mark.asyncio
    async def test_llm_call_async_with_ainvoke(self, mock_graph_retriever, mock_llm_client, mock_llm_response):
        """测试异步 LLM 调用（有 ainvoke 方法）"""
        ...
        result = await retriever._llm_call_async("test prompt")
    ...
    @pytest.mark.asyncio
    async def test_retrieve_with_default_mode(self, mock_graph_retriever, mock_llm_client):
        """测试使用默认模式检索"""
        mock_graph_retriever.retrieve = AsyncMock(return_value=[
            RetrievalResult(text="Result 1", score=0.9),
        ])
        ...
        retriever._default_mode = "hybrid"
