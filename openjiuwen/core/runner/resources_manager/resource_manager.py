    ...
    async def refresh_mcp_server(self,
        ...
        Returns:
            Result[str, Exception] or list[Result[str, Exception]]:
                Result object(s) containing the server name(s) or exception(s).
        """
        server_ids, exact_match = self._inner_get_server_ids(server_id, server_name, tag, tag_match_strategy,
                                                             skip_if_tag_not_exists,
                                                             StatusCode.RESOURCE_MCP_SERVER_REFRESH_ERROR)
        results = []
        for server_id in server_ids:
            try:
                await self._resource_registry.tool().refresh_tool_server(server_id,
                                                                         skip_if_tag_not_exists or not exact_match,
                                                                         force=True)
                results.append(Ok(server_id))
            except Exception as e:
    ...
    def _inner_add_resource(self, *, resource_id: str, resource_type: str, resource: Any,
        ...
        try:
            if self._tag_mgr.has_resource(resource_id):
                raise build_error(StatusCode.RESOURCE_ADD_ERROR, card=resource_card | resource_id,
            ...
            if resource_card:
                logger.error(
                    f"add resource failed, id={resource_id}, type={resource_type}, card={resource_card}, reason={str(e)}")
            else:
