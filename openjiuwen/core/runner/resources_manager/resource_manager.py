...
class ResourceMgr:
    ...
    async def add_mcp_server(self,
        ...
        for config in server_configs:
            try:
                ...
                logger.info(
                    f"add mcp server succeed, id={config.server_id}, server_name={config.server_name}, tools={tool_names}")
    ...
    def _inner_remove_resources(self, *, resource_id: Optional[str | list[str]], resource_type: str,
        ...
        for remove_id in ids_to_remove:
            ...
            if error:
                logger.error(
                    f"remove resource error, id={resource_id}, type={resource_type}, card={removed_card}, reason={str(error)}")
            ...
            if not error:
                if removed_card:
                    logger.info(f"remove resource succeed, id={resource_id}, type={resource_type}, card={removed_card}")
                else:
